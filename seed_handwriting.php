<?php
// Populate Mira's invitation with Handwriting theme demo data
...
use Database\Seeders\HandwritingThemeSeeder;

echo "Running HandwritingThemeSeeder...\n";
$seeder = new HandwritingThemeSeeder();
$seeder->run();
...
$theme = \App\Models\Theme::where('slug', 'handwriting')->first();
if (!$theme) {
    echo "Handwriting theme not found in database!\n";
    exit;
}

echo "Found invitation ID: {$inv->id} — {$inv->title}\n";
echo "Setting theme to 'handwriting' (ID: {$theme->id})...\n";
...
echo "Done! Seeded Handwriting theme demo data to 'mira-randi'\n";
