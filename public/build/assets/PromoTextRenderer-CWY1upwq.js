import{r as x,j as e,R as j}from"./app-BNn5b5pI.js";function u({durationHours:n=19,resellerRef:i="default"}){const[t,r]=x.useState(0);x.useEffect(()=>{const a=`promo_countdown_target_${i}`,w=(()=>{const c=localStorage.getItem(a),l=Date.now();if(c){const g=parseInt(c,10);if(g>l)return g}const m=l+n*60*60*1e3;return localStorage.setItem(a,m.toString()),m})(),f=()=>{const c=Date.now(),l=w-c;if(l<=0){const m=c+n*60*60*1e3;localStorage.setItem(a,m.toString()),r(n*60*60*1e3);return}r(l)};f();const h=setInterval(f,1e3);return()=>clearInterval(h)},[n,i]);const d=Math.floor(t/(1e3*60*60)),p=Math.floor(t%(1e3*60*60)/(1e3*60)),s=Math.floor(t%(1e3*60)/1e3),o=a=>String(a).padStart(2,"0");return e.jsxs("span",{className:"rl-countdown-timer",children:[e.jsx("style",{dangerouslySetInnerHTML:{__html:`
                .rl-countdown-timer {
                    display: inline-flex;
                    align-items: center;
                    gap: 3px;
