<?php
// Populate Mira's invitation with Terracotta Minimalist theme demo data
...
use Database\Seeders\TerracottaMinimalistThemeSeeder;

echo "Running TerracottaMinimalistThemeSeeder...\n";
$seeder = new TerracottaMinimalistThemeSeeder();
$seeder->run();
...
$theme = \App\Models\Theme::where('slug', 'terracotta-minimalist')->first();
if (!$theme) {
    echo "Terracotta Minimalist theme not found in database!\n";
    exit;
}

echo "Found invitation ID: {$inv->id} — {$inv->title}\n";
echo "Setting theme to 'terracotta-minimalist' (ID: {$theme->id})...\n";
...
echo "Done! Seeded Terracotta Minimalist theme demo data to 'mira-randi'\n";
