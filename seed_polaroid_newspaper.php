<?php
// Populate Mira's invitation with Polaroid Newspaper theme demo data
...
use Database\Seeders\PolaroidNewspaperThemeSeeder;

echo "Running PolaroidNewspaperThemeSeeder...\n";
$seeder = new PolaroidNewspaperThemeSeeder();
$seeder->run();
...
$theme = \App\Models\Theme::where('slug', 'polaroid-newspaper')->first();
if (!$theme) {
    echo "Polaroid newspaper theme not found in database!\n";
    exit;
}

echo "Found invitation ID: {$inv->id} — {$inv->title}\n";
echo "Setting theme to 'polaroid-newspaper' (ID: {$theme->id})...\n";
...
echo "Done! Seeded Polaroid Newspaper theme demo data to 'mira-randi'\n";
