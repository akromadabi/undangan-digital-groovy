...
const createTextTexture = (layer, resolvedText) => {
    const canvas = document.createElement('canvas');
    const ctx = canvas.getContext('2d');
    
    const text = resolvedText || layer.text || 'Teks Baru';
    ...
    else if (textEffect === 'neon') effectPadding = Math.max(effectPadding, glowBlur + 10);
    ...
    lines.forEach((line, index) => {
        if (textEffect === 'outline') {
            ...
            ctx.lineJoin = 'round';
            ctx.strokeText(line, canvasWidth / 2, padding + index * lineHeight);
        }
        ctx.fillStyle = color;
...
const resolveBindingData = (invitation, brideGrooms, events, guest) => {
    const bride = brideGrooms?.find(bg => bg.gender === 'wanita');
    const groom = brideGrooms?.find(bg => bg.gender === 'pria');
    
    const primaryEvent = events?.find(e => e.is_primary) || events?.[0];
    
    let formattedDate = primaryEvent?.event_date || '';
    if (formattedDate) {
        try {
    ...
    return {
        bride_full_name: bride?.full_name || '',
        bride_nickname: bride?.nickname || '',
        bride_father_name: bride?.father_name || '',
        bride_mother_name: bride?.mother_name || '',
        bride_child_order: bride?.child_order || '',
        bride_instagram: bride?.instagram ? (bride.instagram.startsWith('@') ? bride.instagram : `@${bride.instagram}`) : '',
        
        groom_full_name: groom?.full_name || '',
        groom_nickname: groom?.nickname || '',
        groom_father_name: groom?.father_name || '',
        groom_mother_name: groom?.mother_name || '',
        groom_child_order: groom?.child_order || '',
        groom_instagram: groom?.instagram ? (groom.instagram.startsWith('@') ? groom.instagram : `@${groom.instagram}`) : '',
        
        event_name: primaryEvent?.event_name || '',
        event_date: formattedDate,
        event_time: primaryEvent ? `${primaryEvent.start_time || ''} s/d ${primaryEvent.end_time || 'Selesai'} ${primaryEvent.timezone || 'WIB'}` : '',
        venue_name: primaryEvent?.venue_name || '',
        venue_address: primaryEvent?.venue_address || '',
        event_dress_code: primaryEvent?.event_dress_code || invitation?.event_dress_code || '',
        
        guest_name: guest?.name || 'Tamu Undangan'
    };
};

const parseTemplateText = (text, data) => {
    if (!text) return '';
    let parsed = text;
...
export default function ThreeDScenePlayer({ 
    ...
    const initParticleSystem = (scene, type) => {
        ...
        const speed = config.particleSpeed || 'normal';
    ...
    const animateLayers = (customTime = null) => {
        ...
        config.layers.forEach(layer => {
            ...
            if (layer.fadeMode && layer.fadeMode !== 'none' && kfs.length >= 2) {
            ...
            if (layer.contents && Array.isArray(layer.contents)) {
                layer.contents.forEach((content, contentIdx) => {
                    ...
                    if (content.fadeMode && content.fadeMode !== 'none' && kfs.length >= 2) {
                        const progressVal = currentCameraProgressRef.current;
                        const k = progressVal * (kfs.length - 1);
                    ...
                    if (content.type === 'text') {
                        ...
                        childMesh.scale.set(content.scale.x * cScaleX, content.scale.y * cScaleY, 1);
                    }
    ...
    useEffect(() => {
        ...
        const bindingData = resolveBindingData(invitation, brideGrooms, events, guest);
        const migratedLayers = migrateLayers(config.layers || []);

        // Load textures and construct meshes grouped by layer
        const textureLoader = new THREE.TextureLoader();
        
        migratedLayers.forEach((layer, idx) => {
            if (!layer.visible) return;

            // Create parent group for the layer
            const group = new THREE.Group();
            const listOffset = (migratedLayers.length - 1 - idx) * 0.05;
            ...
            scene.add(group);
            meshesRef.current[layer.id] = group; // Track group reference for movement / keyframes

            // Render contents inside the group
            if (layer.contents && Array.isArray(layer.contents)) {
                layer.contents.forEach((content, idx) => {
                    ...
                    if (content.type === 'text') {
                        // Resolve dynamic text bindings
                        let textValue = content.text || '';
                        if (content.dataBinding) {
                            textValue = bindingData[content.dataBinding] || textValue;
                        } else {
                            textValue = parseTemplateText(textValue, bindingData);
                        }
                        
                        texture = createTextTexture(content, textValue);
                    } else {
                        texture = textureLoader.load(content.url, (tex) => {
                            // Adjust plane scale based on texture dimensions with aspect ratio clamping
                    ...
                    childMesh.name = content.id;
                    childMesh.userData = {
                        type: content.type,
                        aspectRatio: content.type === 'text' ? texture.aspectRatio : 1
                    };
                    
                    childMesh.position.set(
                        content.position.x || 0, 
                        content.position.y || 0, 
                        (content.position.z || 0) + idx * 0.005
                    );
                    ...
                    if (content.type === 'text') {
                        ...
                        childMesh.scale.set(content.scale.x, content.scale.y, 1);
                    }
        ...
        // Raycasting for Interactive 3D Hotspots
        ...
        const getContentById = (id) => {
            for (const layer of config.layers || []) {
                const found = (layer.contents || []).find(c => c.id === id);
        ...
        const handlePointerMove = (e) => {
            ...
            sceneRef.current.traverse(child => {
                if (child.isMesh && child.name && child.name.startsWith('content_')) {
            ...
            if (intersects.length > 0) {
                ...
                const content = getContentById(hitMesh.name);
                if (content) {
                    const lowerId = String(content.id).toLowerCase();
                    const lowerText = String(content.text || '').toLowerCase();
                    const isRsvp = content.action === 'rsvp' || lowerId.includes('rsvp') || lowerText.includes('rsvp') || lowerText.includes('kehadiran');
                    const isWishes = content.action === 'wishes' || lowerId.includes('wishes') || lowerText.includes('wishes') || lowerText.includes('buku tamu') || lowerText.includes('ucapan');
                    const isCheckin = content.action === 'checkin' || lowerId.includes('checkin') || lowerId.includes('qr') || lowerText.includes('check-in');
                    const isMap = content.action === 'map' || lowerId.includes('map') || lowerId.includes('lokasi') || lowerText.includes('peta') || lowerText.includes('lokasi');
                    
                    if (content.action || isRsvp || isWishes || isCheckin || isMap) {
                        foundInteractive = hitMesh;
                    }
                }
            }
            ...
            if (foundInteractive) {
                ...
                if (hoveredObj !== foundInteractive) {
                    if (hoveredObj) {
                        hoveredObj.scale.copy(originalScale);
                    }
                    
                    hoveredObj = foundInteractive;
                    originalScale.copy(foundInteractive.scale);
                    
                    // Hover effect: scale up gently
                    foundInteractive.scale.set(originalScale.x * 1.08, originalScale.y * 1.08, originalScale.z);
                }
            } else {
        ...
        const handlePointerClick = (e) => {
            if (!containerRef.current || !cameraRef.current || !sceneRef.current || !rendererRef.current || !onTriggerAction) return;
            ...
            if (intersects.length > 0) {
                ...
                if (content) {
                    ...
                    const action = content.action || (isRsvp ? 'rsvp' : isWishes ? 'wishes' : isCheckin ? 'checkin' : 'map');
                    if (action) {
        ...
        canvasEl.addEventListener('touchstart', handlePointerMove, { passive: true });
        canvasEl.addEventListener('touchend', handlePointerClick);
        ...
        return () => {
            ...
            if (canvasEl) {
                ...
                canvasEl.removeEventListener('touchstart', handlePointerMove);
                canvasEl.removeEventListener('touchend', handlePointerClick);
            }
            ...
            // Dispose geometries and materials recursively inside the group
    ...
    }, [config, invitation, brideGrooms, events, guest]);
    ...
    useEffect(() => {
        ...
        const updateCameraPosition = (progress) => {
            ...
            currentCameraProgressRef.current = clampedProgress;
        ...
        if (autoplay) {
            // Autoplay animation with keyframe pause transitions
            let start = null;

            const D_move = 2000; // 2 seconds movement per segment
            const pauseSec = config.keyframePauseDuration ?? 2.0;
            const D_pause = pauseSec * 1000; // pause duration in ms
            const D_segment = D_move + D_pause;

            const totalSegments = kfs.length - 1;
            const duration = totalSegments * D_segment; // total duration
            ...
            const step = (timestamp) => {
                ...
                updateCameraPosition(globalProgress);
    ...
    const handleVideoExport = async () => {
        ...
        // Mix Audio from configuration musicUrl or invitation.music_url
        let audioCtx = null;
        let dest = null;
        let audio = null;
        let mixedStream = new MediaStream();

        // Capture video track from WebGL Canvas
        const canvas = rendererRef.current.domElement;
        const canvasStream = canvas.captureStream(0); // Manual frame capture
        const videoTrack = canvasStream.getVideoTracks()[0];
        mixedStream.addTrack(videoTrack);

        const musicUrl = invitation?.music_url || config.musicUrl || '/audio/backsound.mp3';
        let useAudio = false;
        ...
        try {
            ...
            console.warn("MimeType tidak didukung, menggunakan default browser recorder:", err);
        ...
        recorder.onstop = () => {
            ...
            a.href = url;
            a.download = `${invitation?.slug || 'undangan-3d'}_ekspor.${fileExtension}`;
        ...
        for (let frameIndex = 0; frameIndex <= totalFrames; frameIndex++) {
            ...
            const elapsed = frameIndex * (1000 / FPS); // in ms
            ...
            // Update particles deterministically
            ...
            // Wait exactly 1000/FPS ms to sync with the audio track
    ...
    return (
        <div 
            ...
            style={{ height }}
        >
            {config.keyframes && config.keyframes.length >= 2 && !isExporting && (
                <button
                    type="button"
                    onClick={handleVideoExport}
                    className="absolute top-4 left-4 z-30 pointer-events-auto flex items-center gap-1.5 px-3 py-2 bg-stone-900/80 backdrop-blur-md border border-white/10 hover:bg-stone-850 hover:border-emerald-500/30 text-white rounded-xl text-xs font-bold transition shadow-xl"
                >
            ...
            {isExporting && (
                <div className="absolute inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm p-4">
                    <div className="bg-[#1c1c1f]/95 border border-white/10 rounded-2xl max-w-xs w-full p-5 text-center text-white shadow-2xl relative overflow-hidden pointer-events-auto">
                        <div className="relative z-10 flex flex-col items-center gap-3">
                            ...
                            <div>
                                <h3 className="text-xs font-bold">Mengekspor Undangan Video</h3>
                                <p className="text-[9px] text-stone-400 mt-0.5">Membuat video frame-by-frame berkualitas tinggi...</p>
                            </div>

                            <div className="w-full mt-1">
                                <div className="flex justify-between items-center text-[10px] font-mono text-stone-300 mb-0.5">
                                    <span>Progress</span>
                                    <span className="text-emerald-400">{exportProgress}%</span>
                                </div>
                                <div className="w-full h-1.5 bg-stone-850 rounded-full overflow-hidden border border-white/5">
                                    <div 
                                        className="h-full bg-emerald-500 transition-all duration-100 rounded-full shadow-[0_0_6px_rgba(16,185,129,0.5)]" 
                            ...
                            <button
                                ...
                                className="mt-1 px-3 py-1 bg-white/5 hover:bg-red-500/10 hover:text-red-400 hover:border-red-500/20 text-stone-300 border border-white/10 rounded-lg text-[9px] font-bold transition duration-200"
                            >
                                Batal
                            </button>
                        </div>
