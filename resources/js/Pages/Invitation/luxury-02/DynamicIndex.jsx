    ...
    render() {
        if (this.state.hasError) return (
            <div style={{ padding: 20, color: '#fff', background: '#111', minHeight: '100vh', fontFamily: 'monospace' }}>
                <h2 style={{ color: '#ff4d4d' }}>Terjadi kesalahan pada tema Luxury 02.</h2>
...
function Reveal({ children, className = '', variant = 'up', delay = 0 }) {
    ...
    else if (variant === 'left') baseClass = 'lx2-reveal--left';
    else if (variant === 'right') baseClass = 'lx2-reveal--right';
    else if (variant === 'down') baseClass = 'lx2-reveal--down';
...
function HeartDivider() {
    return (
        <div className="lx2-divider">
            <span className="lx2-divider-inner">
    ...
    return (
        <div className={`lx2-cover${isOpened ? ' is-opened' : ''} ${!globalShowPhotos ? 'lx2-no-photo-mode' : ''}`}>
            ...
            <div className="lx2-cover__content">
                ...
                <div className="lx2-cover__circle-logo">
                    ...
                    <div className="lx2-cover__monogram">{initials}</div>
                </div>

                <h1 className="lx2-cover__couple">{coupleName}</h1>
                <p className="lx2-cover__dear">Dear:</p>
                <div className="lx2-cover__guest">{guestName}</div>
                <p className="lx2-cover__apology">{t('invitation.dear_guest_desc')}</p>

                <button type="button" onClick={onOpen} id="tombol-buka" className="lx2-cover__btn">
 ...
 {list?.filter(p => p.show_dress_code).map((p, idx) => (
    ...
    return (
        <section id="hero" className="lx2-hero">
            {globalShowPhotos && coverEmbedId ? (
                ...
                <>
                    <PremiumSlideshow
                        ...
                        positionY={invitation?.cover_position_y}
                        zoom={invitation?.cover_zoom}
                        className="lx2-hero__bg is-active absolute inset-0 w-full h-full z-0"
                        imgClassName="absolute inset-0 w-full h-full object-cover"
                    />
                    <div className="absolute inset-0 w-full h-full z-[1]" style={{ backgroundColor: 'rgba(0, 0, 0, 0.4)' }} />
                </>
            ...
            <div className="lx2-hero__overlay" />
            <div className="lx2-hero__content">
                <Reveal delay={200}>
                    <span className="lx2-hero__subtitle">{t('invitation.wedding_of').toUpperCase()}</span>
                </Reveal>
                <Reveal delay={400}>
                    <h1 className="lx2-hero__title">{coupleName}</h1>
                </Reveal>
                <Reveal delay={600}>
                    <span className="lx2-hero__date">{t('invitation.save_the_date').toUpperCase()} | {formattedDate}</span>
                </Reveal>
                
                <Reveal delay={800}>
                    <div className="lx2-hero__countdown">
                        <div className="lx2-hero__countdown-item">
                            <span className="lx2-hero__countdown-val">{pad2(timeLeft.d)}</span>
                            <span className="lx2-hero__countdown-label">{t('invitation.days')}</span>
                        </div>
                        <div className="lx2-hero__countdown-item">
                            <span className="lx2-hero__countdown-val">{pad2(timeLeft.h)}</span>
                            <span className="lx2-hero__countdown-label">{t('invitation.hours')}</span>
                        </div>
                        <div className="lx2-hero__countdown-item">
                            <span className="lx2-hero__countdown-val">{pad2(timeLeft.m)}</span>
                            <span className="lx2-hero__countdown-label">{t('invitation.minutes')}</span>
                        </div>
                        <div className="lx2-hero__countdown-item">
                            <span className="lx2-hero__countdown-val">{pad2(timeLeft.s)}</span>
                            <span className="lx2-hero__countdown-label">{t('invitation.seconds')}</span>
                        </div>
                    </div>
                </Reveal>
                
                <div className={`lx2-hero__arrow ${isHorizontal ? 'lx2-hero__arrow--horizontal' : 'lx2-hero__arrow--vertical'}`}>
...
function OpeningSection({ invitation, brideGrooms, openingEmbedId }) {
    ...
    return (
        <section id="opening" className="lx2-section lx2-opening">
            <Reveal>
                {isWeddingOf ? (
                    <div className="lx2-opening__header">
                        <span className="lx2-opening__subtitle">{t('invitation.wedding_of').toUpperCase()}</span>
                        {first && second && (
                            <div className="lx2-opening__initials">
                                <span className="lx2-opening__initials-letter">{first}</span>
                                <span className="lx2-opening__initials-amp">&amp;</span>
                                <span className="lx2-opening__initials-letter">{second}</span>
                            </div>
                        )}
                    </div>
                ) : (
                    <h2 className="lx2-opening__title">{invitation?.opening_title || 'Maha Suci Allah'}</h2>
                )}
                
                {invitation?.opening_ayat && (
                    <div className="lx2-opening__quote-container">
                        <p className="lx2-opening__ayat">&ldquo;{invitation.opening_ayat}&rdquo;</p>
                        {invitation?.opening_ayat_source && (
                            <p className="lx2-opening__source">&mdash; {invitation.opening_ayat_source}</p>
                        )}
                    </div>
                )}
                
                {globalShowPhotos && openingEmbedId ? (
                    <div className="lx2-opening__photo-wrapper relative aspect-[16/9]" style={{ margin: '30px auto', maxWidth: '320px', borderRadius: '16px', overflow: 'hidden', border: '4px solid #fff', boxShadow: '0 8px 24px rgba(0,0,0,0.08)' }}>
                        <iframe
                    ...
                    <div className="lx2-opening__photo-wrapper relative aspect-[4/3]" style={{ margin: '30px auto', maxWidth: '320px', borderRadius: '16px', overflow: 'hidden', border: '4px solid #fff', boxShadow: '0 8px 24px rgba(0,0,0,0.08)' }}>
                        <PremiumSlideshow
                ...
                <p className="lx2-opening__text" style={{ marginTop: '30px' }}>
...
function BrideGroomSection({ brideGrooms }) {
    ...
    return (
        <section id="bride_groom" className="lx2-section lx2-couple">
            <Reveal variant="up">
                <h2 className="lx2-section-title">{t('invitation.mempelai')}</h2>
                <p className="lx2-section-subtitle">Groom &amp; Bride</p>
            </Reveal>
            ...
            <Reveal variant="zoom">
                <div className="lx2-mempelai-amp">&amp;</div>
            </Reveal>
    ...
    return (
        <div className="lx2-countdown-wrapper" style={{ position: 'relative', overflow: 'hidden', padding: '60px 24px', borderBottom: '1px solid rgba(197, 168, 128, 0.08)' }}>
            ...
            <div style={{ position: 'relative', zIndex: 2 }}>
                <Reveal>
                    <h2 className="lx2-section-title">{t('invitation.save_the_date')}</h2>
                    <p className="lx2-section-subtitle">{t('invitation.save_the_date') === 'Save The Date' ? 'Counting Down' : 'Menghitung Hari'}</p>
                </Reveal>

                <Reveal variant="zoom">
                    <div className="lx2-countdown">
                        {/* Days */}
                        <div className="lx2-countdown__item">
                            <div className="lx2-countdown__value">
                                {dDigits.map((digit, idx) => (
                                    <span key={`d-${idx}`} className="lx2-countdown__digit">{digit}</span>
                                ))}
                            </div>
                            <span className="lx2-countdown__label">{t('invitation.days')}</span>
                        </div>

                        <span className="lx2-countdown__colon">:</span>

                        {/* Hours */}
                        <div className="lx2-countdown__item">
                            <div className="lx2-countdown__value">
                                {hDigits.map((digit, idx) => (
                                    <span key={`h-${idx}`} className="lx2-countdown__digit">{digit}</span>
                                ))}
                            </div>
                            <span className="lx2-countdown__label">{t('invitation.hours')}</span>
                        </div>
                        ...
                        <div className="lx2-countdown__item">
                            <div className="lx2-countdown__value">
                                {mDigits.map((digit, idx) => (
                                    <span key={`m-${idx}`} className="lx2-countdown__digit">{digit}</span>
                                ))}
                            </div>
                            <span className="lx2-countdown__label">{t('invitation.minutes')}</span>
                        </div>
                        ...
                        <div className="lx2-countdown__item">
                            <div className="lx2-countdown__value">
                                {sDigits.map((digit, idx) => (
                                    <span key={`s-${idx}`} className="lx2-countdown__digit">{digit}</span>
                                ))}
                            </div>
                            <span className="lx2-countdown__label">{t('invitation.seconds')}</span>
                        </div>
                    </div>
                </Reveal>
            </div>
...
function EventSection({ events, galleries, showCountdown }) {
    ...
    return (
        <section id="event" className="lx2-section" style={{ padding: showCountdown ? '0 0 60px 0' : '60px 24px' }}>
            {showCountdown && (
            ...
            <div style={{ padding: showCountdown ? '60px 24px 0 24px' : '0' }}>
                <Reveal>
                    <h2 className="lx2-section-title">{t('nav.acara')}</h2>
                    <p className="lx2-section-subtitle">{t('invitation.wedding_events')}</p>
                </Reveal>
                ...
                {list.map((evt, idx) => {
                    ...
                    return (
                        <Reveal key={evt.id || idx} variant={isEven ? 'left' : 'right'} className="lx2-event-card-ref">
                            ...
                            <div className="lx2-event-body-ref">
                                {/* Left column: Dark gray vertical name */}
                                <div className="lx2-event-left-ref">
                                    <div className="lx2-event-vertical-title">{eventDisplayName}</div>
                                </div>
                                {/* Right column: details */}
                                <div className="lx2-event-right-ref">
                                    {/* Date row */}
                                    <div className="lx2-event-date-row-ref">
                                        <span className="lx2-event-date-num-ref">{dayNum}</span>
                                        <div className="lx2-event-date-col-ref">
                                            <span className="lx2-event-date-day-ref">{dayName}</span>
                                            <span className="lx2-event-date-month-ref">{monthName}</span>
                                            <span className="lx2-event-date-year-ref">{year}</span>
                                        </div>
                                    </div>
                                    <hr className="lx2-event-divider-ref" />
                                    {/* Time */}
                                    <p className="lx2-event-time-ref">
                                    ...
                                    {/* Location */}
                                    <div className="lx2-event-location-ref">
                                        <h4 className="lx2-event-location-title-ref">{t('invitation.save_the_date') === 'Save The Date' ? 'Event Location' : 'Lokasi Acara'}</h4>
                                        <p className="lx2-event-venue-ref">{t('invitation.save_the_date') === 'Save The Date' ? 'Venue' : 'Tempat'} : <strong>{evt.venue_name}</strong></p>
                                        <p className="lx2-event-address-ref">{evt.venue_address}</p>
                                    </div>
                                    {/* Maps Button */}
                                    {evt.gmaps_link && (
                                        <a
                                ...
                                {list?.filter(evt => evt.show_dress_code).map((evt, idx) => (
                                    <div key={`dc-${idx}`} className="lx2-event-card-ref w-full mt-4" style={{ padding: '24px', backgroundColor: 'rgba(255,255,255,0.02)', border: '1px solid rgba(197, 168, 128, 0.2)' }}>
                                        <DressCodeBlock event={evt} colors={{ primary: '#c5a880', text: '#4a4a4a' }} fonts={{ heading: 'inherit' }} variant="modern" plain={true} />
                                    </div>
                                ))}
            </div>
        </section>
...
function LoveStorySection({ loveStories }) {
    ...
    return (
        <section id="love_story" className="lx2-section">
            <Reveal>
                <h2 className="lx2-section-title">{t('invitation.love_story')}</h2>
                <p className="lx2-section-subtitle">Our Love Story</p>
            </Reveal>

            <div className="lx2-story">
                {list.map((story, idx) => {
                    const isEven = idx % 2 === 0;
                    return (
...
function GallerySection({ galleries }) {
    ...
    return (
        <section id="gallery" className="lx2-section">
            <Reveal>
                <h2 className="lx2-section-title">{t('invitation.gallery')}</h2>
                <p className="lx2-section-subtitle">Our Moments</p>
            </Reveal>

            <div className="lx2-gallery-grid">
                {list.map((photo, idx) => {
                    ...
                    return (
                        <Reveal
                            key={photo.id || idx}
                            variant="zoom"
                            className="lx2-gallery-grid__item"
                        >
                            <img
                                src={imgUrl}
            ...
            {activeImg && (
                <div className="lx2-lightbox" onClick={() => setActiveImg(null)}>
                    <button
                        type="button"
                        className="lx2-lightbox__close"
...
function VideoGallerySection({ invitation, locale }) {
    ...
    return (
        <section id="video" className="lx2-section">
            <Reveal>
                <h2 className="lx2-section-title">{locale === 'en' ? 'Video Gallery' : 'Galeri Video'}</h2>
                <p className="lx2-section-subtitle">Our Video Moments</p>
            </Reveal>
                ...
                {videoItems.map((item, idx) => (
                    <Reveal key={idx} variant="zoom" delay={idx * 50} className="w-full">
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                            {videoItems.length > 1 && (
                                <h4 style={{ color: 'var(--lx2-text-dark, #333)', fontSize: '0.9rem', fontWeight: 'bold', margin: '0', letterSpacing: '0.5px' }}>
                                    {item.title}
                                </h4>
                            )}
                            <div 
                                style={{
                                    ...
                                    overflow: 'hidden',
                                    borderRadius: '12px',
                                    border: '1px solid var(--lx2-gold-border, rgba(197, 168, 128, 0.3))',
...
function BankSection({ bankAccounts, invitation }) {
    ...
    return (
        <section id="bank" className="lx2-section">
            <Reveal>
                <h2 className="lx2-section-title">{t('nav.bank')}</h2>
                <p className="lx2-section-subtitle">{t('invitation.gift_title')}</p>
            </Reveal>

            <div className="lx2-bank-cards" style={{ width: '100%', display: 'flex', flexDirection: 'column', gap: '24px' }}>
                {list.map((acc, idx) => {
                    const isBca = acc.bank_name?.toLowerCase().includes('bca');
                    const isDana = acc.bank_name?.toLowerCase().includes('dana');

                    return (
                        <Reveal key={acc.id || idx} className="lx2-bank-card">
                            <div className="lx2-bank-card__header">
                                {isBca && <img src={ASSETS.bca} alt="BCA" className="lx2-bank-card__logo" />}
                                {isDana && <img src={ASSETS.dana} alt="DANA" className="lx2-bank-card__logo" />}
                                {!isBca && !isDana && <span style={{ fontWeight: 600, fontSize: '1.1rem', zIndex: 2 }}>{acc.bank_name}</span>}
                                <img src={ASSETS.chip} alt="Chip" className="lx2-bank-card__chip" />
                            </div>
                            <div className="lx2-bank-card__body">
                                <div className="lx2-bank-card__number">{acc.account_number}</div>
                                <div className="lx2-bank-card__holder">{acc.account_holder || acc.account_name}</div>
                            </div>
                            <div className="lx2-bank-card__footer">
                                <button
                                    type="button"
                                    className="lx2-bank-card__copy-btn"
...
function Navigation({
    ...
    useEffect(() => {
        ...
        const activeEl = document.querySelector(`.lx2-nav-menu button[data-id="${activeMenuId}"]`);
        if (activeEl) {
    ...
    return (
        <>
            {/* Dock Menu */}
            {showMenu && (
                <div className={`lx2-nav-menu ${isBottomMenu ? 'lx2-nav-menu--bottom' : 'lx2-nav-menu--top'}`}>
                    <div className="lx2-nav-menu__inner--row">
                        {menuItems.map((item) => {
                            ...
                            return (
                                <button
                                    ...
                                    className={`lx2-nav-menu__item ${isActive ? 'active' : ''}`}
                                    title={item.label}
                                >
                                    <i className={item.icon} />
                                    <span className="lx2-nav-item-text">{item.label}</span>
                                </button>
                            );
                        })}
                    </div>
            ...
            <div className={`lx2-floating-btns ${isBottomMenu ? 'lx2-floating-btns--raised' : ''}`}>
                {enableQr && activeGuest && (
                    <button
                        type="button"
                        className="lx2-floating-btn"
                ...
                <button
                    ...
                    style={isFullscreen ? { backgroundColor: 'var(--lx2-gold, #c5a880)', color: '#fff', boxShadow: '0 0 10px var(--lx2-gold)' } : {}}
                ...
                <button
                    ...
                    style={autoScrollEnabled ? { backgroundColor: 'var(--lx2-gold, #c5a880)', color: '#fff', boxShadow: '0 0 10px var(--lx2-gold)' } : {}}
            ...
            {enableQr && showQr && activeGuest && (
                <div className="lx2-qr-overlay" onClick={() => setShowQr(false)}>
                    <div className="lx2-qr-modal" onClick={e => e.stopPropagation()}>
                        <h3 className="lx2-qr-title">QR Code Check-in</h3>
                        <p className="lx2-qr-guest">{activeGuest.name}</p>
                        <div className="lx2-qr-img-wrap">
                            <img
                                ...
                                alt="QR Code"
                                className="lx2-qr-img"
                            />
                        </div>
                        <p className="lx2-qr-hint">Tunjukkan QR Code ini kepada penerima tamu di lokasi acara</p>
                        <button
                            type="button"
                            className="lx2-qr-close"
...
export default function DynamicIndex({
    ...
    const resolvedSections = useMemo(() => {
        ...
        return resolved;
    }, [sections, events, loveStories, galleries, bankAccounts, enableRsvp, enableWishes, showCountdown, showPhotos, hasVideos]);
    ...
    useEffect(() => {
        ...
        const handleUserInteraction = (e) => {
            if (
                ...
                e.target.closest('.lx2-floating-btn') || 
                e.target.closest('.lx2-nav-menu') || 
    ...
    const handlePointerDown = (clientX, clientY, target) => {
        ...
        if (target && typeof target.closest === 'function') {
            const scrollable = target.closest('.lx2-slide-container');
            if (scrollable) {
    ...
    const handlePointerUp = (clientX, clientY, target) => {
        ...
        if (layoutMode === 'slide-h') {
            ...
            if ((Math.abs(diffY) > Math.abs(diffX)) && (Math.abs(diffY) > threshold || isFastSwipe)) {
                const scrollable = target && typeof target.closest === 'function' ? target.closest('.lx2-slide-container') : null;
                if (scrollable) {
    ...
    const handleWheel = (e) => {
        ...
        const target = e.target.closest('.lx2-slide-container');
    ...
    const renderSectionComponent = (section, idx) => {
        ...
        if (isSlideMode) {
            let slideClass = 'lx2-slide-container';
    ...
    return (
        <ErrorBoundary>
            ...
            <div className="lx2-container">
                <div
                    className={`lx2-main ${isSlideMode ? 'lx2-main--slide' : ''} ${layoutMode === 'slide-h' ? 'lx2-main--slide-h' : ''} ${layoutMode === 'slide-v' ? 'lx2-main--slide-v' : ''}`}
            ...
            {waNumber && isOpened && (
                <div className={`lx2-whatsapp-float ${isBottomMenu ? 'lx2-whatsapp-float--raised' : ''}`}>
                    <a
                        href={`https://wa.me/${waNumber.replace(/[^0-9]/g, '')}`}
                        target="_blank"
                        rel="noopener noreferrer"
                        className="lx2-whatsapp-btn"
