...
import gununganImg from './asset/gunungan.webp';
...
const ASSETS = {
    ...
    groom: defaultGroom,
    bride: defaultBride,
    gunungan: gununganImg,
    ornamenAtas: ornamenAtas,
    ornamenBawah: ornamenBawah,
    ornamenBunga: ornamenBunga,
};
    ...
    render() {
        if (this.state.hasError) return (
            <div style={{ padding: 20, color: '#fff', background: '#111', minHeight: '100vh', fontFamily: 'monospace' }}>
                <h2 style={{ color: '#ff4d4d' }}>Terjadi kesalahan pada tema Jawa New.</h2>
...
function Reveal({ children, className = '', variant = 'up', delay = 0 }) {
    ...
    let baseClass = 'jwn-reveal--up';
    if (variant === 'zoom') baseClass = 'jwn-reveal--zoom';
    else if (variant === 'left') baseClass = 'jwn-reveal--left';
    else if (variant === 'right') baseClass = 'jwn-reveal--right';
    else if (variant === 'down') baseClass = 'jwn-reveal--down';
...
function HeartDivider() {
    return (
        <div className="jwn-divider">
            <span className="jwn-divider-inner" style={{ background: '#1A0F0A', padding: '0 12px' }}>
                <img src={ASSETS.gunungan} alt="gunungan" style={{ width: 28, height: 'auto', display: 'inline-block', filter: 'drop-shadow(0 2px 4px rgba(0,0,0,0.5))' }} />
            </span>
        </div>
    );
}
...
function CoverSection({ invitation, brideGrooms, guest, isOpened, onOpen }) {
    ...
    return (
        <div className={`jwn-cover${isOpened ? ' is-opened' : ''} ${!globalShowPhotos ? 'jwn-no-photo-mode' : ''}`}>
            {/* Javanese corner / border ornaments */}
            <div className="jwn-cover-ornament jwn-cover-ornament--top" style={{ position: 'absolute', top: '10px', left: '50%', transform: 'translateX(-50%)', width: '100%', maxWidth: '280px', pointerEvents: 'none', zIndex: 3 }}>
                <img src={ASSETS.ornamenAtas} alt="ornament" style={{ width: '100%', height: 'auto', opacity: 0.8 }} />
            </div>
            <div className="jwn-cover-ornament jwn-cover-ornament--bottom" style={{ position: 'absolute', bottom: '20px', left: '50%', transform: 'translateX(-50%)', width: '100%', maxWidth: '280px', pointerEvents: 'none', zIndex: 3 }}>
                <img src={ASSETS.ornamenBawah} alt="ornament" style={{ width: '100%', height: 'auto', opacity: 0.8 }} />
            </div>

            {globalShowPhotos && <div className="jwn-cover__bg" style={{ backgroundImage: `url(${coverBg})` }} />}
            <div className="jwn-cover__overlay" />
            <div className="jwn-cover__content">
                {/* Circular Text Path Logo */}
                <div className="jwn-cover__circle-logo">
                    ...
                    <div className="jwn-cover__monogram">{initials}</div>
                </div>

                <h1 className="jwn-cover__couple">{coupleName}</h1>
                <p className="jwn-cover__dear">Dear:</p>
                <div className="jwn-cover__guest">{guestName}</div>
                <p className="jwn-cover__apology">{t('invitation.dear_guest_desc')}</p>

                <button type="button" onClick={onOpen} id="tombol-buka" className="jwn-cover__btn">
...
function HeroSection({ invitation, brideGrooms, events, galleries, layoutMode }) {
    ...
    return (
        <section id="hero" className="jwn-hero">
            {globalShowPhotos && pics.map((src, idx) => (
                <div
                    key={src}
                    className={`jwn-hero__bg ${idx === bgIdx ? 'is-active' : ''}`}
                    style={{ backgroundImage: `url(${src})` }}
                />
            ))}
            <div className="jwn-hero__overlay" />
            <div className="jwn-hero__content">
                <Reveal delay={200}>
                    <span className="jwn-hero__subtitle">{t('invitation.wedding_of').toUpperCase()}</span>
                </Reveal>
                <Reveal delay={400}>
                    <h1 className="jwn-hero__title">{coupleName}</h1>
                </Reveal>
                <Reveal delay={600}>
                    <span className="jwn-hero__date">{t('invitation.save_the_date').toUpperCase()} | {formattedDate}</span>
                </Reveal>
                
                <Reveal delay={800}>
                    <div className="jwn-hero__countdown">
                        <div className="jwn-hero__countdown-item">
                            <span className="jwn-hero__countdown-val">{pad2(timeLeft.d)}</span>
                            <span className="jwn-hero__countdown-label">{t('invitation.days')}</span>
                        </div>
                        <div className="jwn-hero__countdown-item">
                            <span className="jwn-hero__countdown-val">{pad2(timeLeft.h)}</span>
                            <span className="jwn-hero__countdown-label">{t('invitation.hours')}</span>
                        </div>
                        <div className="jwn-hero__countdown-item">
                            <span className="jwn-hero__countdown-val">{pad2(timeLeft.m)}</span>
                            <span className="jwn-hero__countdown-label">{t('invitation.minutes')}</span>
                        </div>
                        <div className="jwn-hero__countdown-item">
                            <span className="jwn-hero__countdown-val">{pad2(timeLeft.s)}</span>
                            <span className="jwn-hero__countdown-label">{t('invitation.seconds')}</span>
                        </div>
                    </div>
                </Reveal>
                
                <div className={`jwn-hero__arrow ${isHorizontal ? 'jwn-hero__arrow--horizontal' : 'jwn-hero__arrow--vertical'}`}>
...
function OpeningSection({ invitation, brideGrooms }) {
    ...
    return (
        <section id="opening" className="jwn-section jwn-opening">
            <Reveal>
                {isWeddingOf ? (
                    <div className="jwn-opening__header">
                        <span className="jwn-opening__subtitle">{t('invitation.wedding_of').toUpperCase()}</span>
                        {first && second && (
                            <div className="jwn-opening__initials">
                                <span className="jwn-opening__initials-letter">{first}</span>
                                <span className="jwn-opening__initials-amp">&amp;</span>
                                <span className="jwn-opening__initials-letter">{second}</span>
                            </div>
                        )}
                    </div>
                ) : (
                    <h2 className="jwn-opening__title">{invitation?.opening_title || 'Maha Suci Allah'}</h2>
                )}
                
                {invitation?.opening_ayat && (
                    <div className="jwn-opening__quote-container">
                        <p className="jwn-opening__ayat">&ldquo;{invitation.opening_ayat}&rdquo;</p>
                        {invitation?.opening_ayat_source && (
                            <p className="jwn-opening__source">&mdash; {invitation.opening_ayat_source}</p>
                        )}
                    </div>
                )}
                
                <p className="jwn-opening__text" style={{ marginTop: '30px' }}>
...
function BrideGroomSection({ brideGrooms }) {
    ...
    return (
        <section id="bride_groom" className="jwn-section jwn-couple">
            <Reveal variant="up">
                <h2 className="jwn-section-title">{t('invitation.mempelai')}</h2>
                <p className="jwn-section-subtitle">Groom &amp; Bride</p>
            </Reveal>

            {/* Groom */}
            <Reveal className="jwn-mempelai-card" variant="left">
                {globalShowPhotos && groom.photo && (
                    <div className="jwn-mempelai-photo-wrap">
                        <img src={groomPhoto} alt={groom.full_name || 'Groom'} className="jwn-mempelai-photo" />
                    </div>
                )}
                <div className="jwn-mempelai-details">
                    <h3 className="jwn-mempelai-name">{groom.full_name || 'Nama Lengkap Pria'}</h3>
                    <p className="jwn-mempelai-parent-label">
                        {translateChildOrder(groom.child_order, 'pria')}
                    </p>
                    <p className="jwn-mempelai-parents">
            ...
            <Reveal variant="zoom">
                <div className="jwn-mempelai-amp">&amp;</div>
            </Reveal>

            {/* Bride */}
            <Reveal className="jwn-mempelai-card" variant="right">
                {globalShowPhotos && bride.photo && (
                    <div className="jwn-mempelai-photo-wrap">
                        <img src={bridePhoto} alt={bride.full_name || 'Bride'} className="jwn-mempelai-photo" />
                    </div>
                )}
                <div className="jwn-mempelai-details">
                    <h3 className="jwn-mempelai-name">{bride.full_name || 'Nama Lengkap Wanita'}</h3>
                    <p className="jwn-mempelai-parent-label">
...
function CountdownSection({ events, galleries }) {
    ...
    return (
        <div className="jwn-countdown-wrapper" style={{ position: 'relative', overflow: 'hidden', padding: '60px 24px', borderBottom: '1px solid rgba(197, 168, 128, 0.08)' }}>
            {globalShowPhotos && bgImg && (
                <>
                    <div className="jwn-section-bg" style={{ backgroundImage: `url(${bgImg})` }} />
                    <div className="jwn-section-overlay" />
                </>
            )}
            <div style={{ position: 'relative', zIndex: 2 }}>
                <Reveal>
                    <h2 className="jwn-section-title">{t('invitation.save_the_date')}</h2>
                    <p className="jwn-section-subtitle">{t('invitation.save_the_date') === 'Save The Date' ? 'Counting Down' : 'Menghitung Hari'}</p>
                </Reveal>

                <Reveal variant="zoom">
                    <div className="jwn-countdown">
                        {/* Days */}
                        <div className="jwn-countdown__item">
                            <div className="jwn-countdown__value">
                                {dDigits.map((digit, idx) => (
                                    <span key={`d-${idx}`} className="jwn-countdown__digit">{digit}</span>
                                ))}
                            </div>
                            <span className="jwn-countdown__label">{t('invitation.days')}</span>
                        </div>

                        <span className="jwn-countdown__colon">:</span>

                        {/* Hours */}
                        <div className="jwn-countdown__item">
                            <div className="jwn-countdown__value">
                                {hDigits.map((digit, idx) => (
                                    <span key={`h-${idx}`} className="jwn-countdown__digit">{digit}</span>
                                ))}
                            </div>
                            <span className="jwn-countdown__label">{t('invitation.hours')}</span>
                        </div>
                        ...
                        <div className="jwn-countdown__item">
                            <div className="jwn-countdown__value">
                                {mDigits.map((digit, idx) => (
                                    <span key={`m-${idx}`} className="jwn-countdown__digit">{digit}</span>
                                ))}
                            </div>
                            <span className="jwn-countdown__label">{t('invitation.minutes')}</span>
                        </div>
                        ...
                        <div className="jwn-countdown__item">
                            <div className="jwn-countdown__value">
                                {sDigits.map((digit, idx) => (
                                    <span key={`s-${idx}`} className="jwn-countdown__digit">{digit}</span>
                                ))}
                            </div>
                            <span className="jwn-countdown__label">{t('invitation.seconds')}</span>
                        </div>
                    </div>
                </Reveal>
            </div>
...
function EventSection({ events, galleries, showCountdown }) {
    ...
    return (
        <section id="event" className="jwn-section" style={{ padding: showCountdown ? '0 0 60px 0' : '60px 24px' }}>
            {showCountdown && (
            ...
            <div style={{ padding: showCountdown ? '60px 24px 0 24px' : '0' }}>
                <Reveal>
                    <h2 className="jwn-section-title">{t('nav.acara')}</h2>
                    <p className="jwn-section-subtitle">{t('invitation.wedding_events')}</p>
                </Reveal>
                ...
                {list.map((evt, idx) => {
                    ...
                    return (
                        <Reveal key={evt.id || idx} variant={isEven ? 'left' : 'right'} className="jwn-event-card-ref">
                            {globalShowPhotos && eventImg && (
                                <div className="jwn-event-banner-ref">
                                    <img src={eventImg} alt={evt.event_name} />
                                </div>
                            )}
                            <div className="jwn-event-body-ref">
                                {/* Left column: Dark gray vertical name */}
                                <div className="jwn-event-left-ref">
                                    <div className="jwn-event-vertical-title">{eventDisplayName}</div>
                                </div>
                                {/* Right column: details */}
                                <div className="jwn-event-right-ref">
                                    {/* Date row */}
                                    <div className="jwn-event-date-row-ref">
                                        <span className="jwn-event-date-num-ref">{dayNum}</span>
                                        <div className="jwn-event-date-col-ref">
                                            <span className="jwn-event-date-day-ref">{dayName}</span>
                                            <span className="jwn-event-date-month-ref">{monthName}</span>
                                            <span className="jwn-event-date-year-ref">{year}</span>
                                        </div>
                                    </div>
                                    <hr className="jwn-event-divider-ref" />
                                    {/* Time */}
                                    <p className="jwn-event-time-ref">
                                    ...
                                    {/* Location */}
                                    <div className="jwn-event-location-ref">
                                        <h4 className="jwn-event-location-title-ref">{t('invitation.save_the_date') === 'Save The Date' ? 'Event Location' : 'Lokasi Acara'}</h4>
                                        <p className="jwn-event-venue-ref">{t('invitation.save_the_date') === 'Save The Date' ? 'Venue' : 'Tempat'} : <strong>{evt.venue_name}</strong></p>
                                        <p className="jwn-event-address-ref">{evt.venue_address}</p>
                                    </div>
                                    {/* Maps Button */}
                                    {evt.gmaps_link && (
                                        <a
...
function TimelineCard({ story, idx, isEven }) {
    ...
    return (
        <div ref={ref} className={`jwn-story__node jwn-story__node--${isEven ? 'left' : 'right'} ${isActive ? 'is-active' : ''}`}>
            <Reveal className="jwn-story__card">
                <div className="jwn-story__date">
                ...
                <h3 className="jwn-story__title">{story.title}</h3>
                <p className="jwn-story__desc">{story.description || story.story}</p>
            </Reveal>
        </div>
    );
}
...
function LoveStorySection({ loveStories }) {
    ...
    return (
        <section id="love_story" className="jwn-section">
            <Reveal>
                <h2 className="jwn-section-title">{t('invitation.love_story')}</h2>
                <p className="jwn-section-subtitle">Our Love Story</p>
            </Reveal>

            <div className="jwn-story">
                {list.map((story, idx) => {
                    const isEven = idx % 2 === 0;
                    return (
...
function GallerySection({ galleries }) {
    ...
    return (
        <section id="gallery" className="jwn-section">
            <Reveal>
                <h2 className="jwn-section-title">{t('invitation.gallery')}</h2>
                <p className="jwn-section-subtitle">Our Moments</p>
            </Reveal>

            <div className="jwn-gallery-grid">
                {list.map((photo, idx) => {
                    ...
                    return (
                        <Reveal
                            key={photo.id || idx}
                            variant="zoom"
                            className="jwn-gallery-grid__item"
                        >
                            <img
                                src={imgUrl}
            ...
            {activeImg && (
                <div className="jwn-lightbox" onClick={() => setActiveImg(null)}>
                    <button
                        type="button"
                        className="jwn-lightbox__close"
...
function BankSection({ bankAccounts, invitation }) {
    ...
    return (
        <section id="bank" className="jwn-section">
            <Reveal>
                <h2 className="jwn-section-title">{t('nav.bank')}</h2>
                <p className="jwn-section-subtitle">{t('invitation.gift_title')}</p>
            </Reveal>
            ...
            {list.map((acc, idx) => {
                ...
                return (
                    <Reveal key={acc.id || idx} className="jwn-bank-card">
                        <img src={ASSETS.chip} alt="atm chip" className="jwn-bank-card__chip" />
                        <div className="jwn-bank-card__header">
                            <img src={logo} alt={acc.bank_name} className="jwn-bank-card__logo" />
                        </div>
                        <div className="jwn-bank-card__number">{acc.account_number}</div>
                        <div className="jwn-bank-card__holder-label">{invitation?.language === 'en' ? 'Account Holder:' : 'Pemilik Rekening:'}</div>
                        <div className="jwn-bank-card__holder">{acc.account_holder || acc.account_name}</div>

                        <button
                            type="button"
                            className="jwn-bank-card__copy-btn"
...
function UnifiedRsvpWishes({ invitation, wishes, guest, enableRsvp, enableWishes }) {
    ...
    return (
        <section id="rsvp" className="jwn-section">
            <Reveal>
                <h2 className="jwn-section-title">{sectionTitle}</h2>
                <p className="jwn-section-subtitle">
                    {isEn
            ...
            <Reveal>
                <div id="rsvp_container" className="jwn-card" style={{ maxWidth: '500px', margin: '0 auto' }}>
                    <form onSubmit={handleSubmit} className="jwn-rsvp-form">
                        {/* Nama */}
                        <div className="jwn-form-group">
                        ...
                        {success && (
                            <p style={{ color: 'var(--jwn-gold)', marginTop: '12px', fontSize: '13px', textAlign: 'center' }}>
                    ...
                    {enableWishes && recentWishes.length > 0 && (
                        <div className="jwn-wishes-list-wrapper" style={{ marginTop: '20px' }}>
                            <div className="jwn-wishes-list" style={{ maxHeight: '280px', overflowY: 'auto' }}>
                                {recentWishes.map((w, idx) => (
                                    <div key={w.id || idx} className="jwn-wish-bubble">
                                        <div className="jwn-wish-bubble__header">
                                            <span className="jwn-wish-bubble__sender">{w.sender_name || w.name}</span>
                                            {w.attendance && (
                                                <span className={`jwn-wish-bubble__badge ${w.attendance === 'hadir' ? 'jwn-wish-bubble__badge--hadir' : 'jwn-wish-bubble__badge--tidak'}`}>
                                                    {w.attendance === 'hadir' ? t('invitation.rsvp_hadir') : t('invitation.rsvp_tidak_hadir')}
                                                </span>
                                            )}
                                        </div>
                                        <p className="jwn-wish-bubble__message">{w.message}</p>
                                    </div>
                                ))}
                            </div>
                        </div>
...
function LiveStreamingSection({ events, invitation }) {
    ...
    return (
        <section id="livestream" className="jwn-section jwn-livestream-section" style={{ padding: '60px 24px' }}>
            <Reveal>
                <h2 className="jwn-section-title">LIVE STREAMING</h2>
                <p className="jwn-section-subtitle">{isEn ? 'Virtual Wedding Celebration' : 'Prosesi Pernikahan Virtual'}</p>
            </Reveal>

            <Reveal className="jwn-card" style={{ maxWidth: '480px', margin: '0 auto', textAlign: 'center', padding: '30px 20px' }}>
                <h3 className="jwn-event-card__name" style={{ color: 'var(--jwn-gold)', fontSize: '18px', marginBottom: 15 }}>
                ...
                <p style={{ fontSize: '12px', color: 'var(--jwn-text-muted)', lineHeight: '1.6', marginBottom: '25px' }}>
                    {isEn 
                    ...
                    {streamsList.map((stream, idx) => (
                        <a 
                            ...
                            style={{ 
                                ...
                                backgroundColor: 'var(--jwn-gold, #c5a880)',
                                color: '#000',
                                fontWeight: '600',
                                display: 'inline-flex',
                                alignItems: 'center',
...
function ClosingSection({ invitation, brideGrooms, galleries }) {
    ...
    return (
        <section id="closing" className="jwn-section jwn-closing" style={{ position: 'relative', overflow: 'hidden' }}>
            {globalShowPhotos && bgImg && (
                <>
                    <div className="jwn-section-bg" style={{ backgroundImage: `url(${bgImg})` }} />
                    <div className="jwn-section-overlay" />
            ...
            <div style={{ position: 'relative', zIndex: 2 }}>
                <Reveal>
                    <h2 className="jwn-closing__title">{displayClosingTitle}</h2>
                    <p className="jwn-closing__text">
                        {displayClosingText}
                    </p>
                    <HeartDivider />
                    
                    {/* Formal Tanda Tangan Penutup */}
                    <div className="jwn-closing__signature" style={{ marginTop: '25px', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '8px', marginBottom: '25px' }}>
                        <div style={{ fontWeight: '600', letterSpacing: '1px', textTransform: 'uppercase', fontSize: '0.75rem', color: 'var(--jwn-gold)' }}>
                    ...
                    <h3 className="jwn-closing__couple">{coupleName}</h3>
                    <p style={{ marginTop: 40, fontSize: 10, color: 'var(--jwn-text-muted)', letterSpacing: 1 }}>
...
function Navigation({
    ...
    useEffect(() => {
        ...
        const activeEl = document.querySelector(`.jwn-nav-menu button[data-id="${activeMenuId}"]`);
        if (activeEl) {
    ...
    return (
        <>
            {/* Dock Menu */}
            {showMenu && (
                <div className={`jwn-nav-menu ${isBottomMenu ? 'jwn-nav-menu--bottom' : 'jwn-nav-menu--top'}`}>
                    <div className="jwn-nav-menu__inner--row">
                        {menuItems.map((item) => {
                            ...
                            return (
                                <button
                                    ...
                                    className={`jwn-nav-menu__item ${isActive ? 'active' : ''}`}
                                    title={item.label}
                                >
                                    <i className={item.icon} />
                                    <span className="jwn-nav-item-text">{item.label}</span>
                                </button>
                            );
                        })}
                    </div>
            ...
            <div className={`jwn-floating-btns ${isBottomMenu ? 'jwn-floating-btns--raised' : ''}`}>
                {enableQr && activeGuest && (
                    <button
                        type="button"
                        className="jwn-floating-btn"
                ...
                <button
                    ...
                    style={isFullscreen ? { backgroundColor: 'var(--jwn-gold, #c5a880)', color: '#fff', boxShadow: '0 0 10px var(--jwn-gold)' } : {}}
                ...
                <button
                    ...
                    style={autoScrollEnabled ? { backgroundColor: 'var(--jwn-gold, #c5a880)', color: '#fff', boxShadow: '0 0 10px var(--jwn-gold)' } : {}}
            ...
            {enableQr && showQr && activeGuest && (
                <div className="jwn-qr-overlay" onClick={() => setShowQr(false)}>
                    <div className="jwn-qr-modal" onClick={e => e.stopPropagation()}>
                        <h3 className="jwn-qr-title">QR Code Check-in</h3>
                        <p className="jwn-qr-guest">{activeGuest.name}</p>
                        <div className="jwn-qr-img-wrap">
                            <img
                                ...
                                alt="QR Code"
                                className="jwn-qr-img"
                            />
                        </div>
                        <p className="jwn-qr-hint">Tunjukkan QR Code ini kepada penerima tamu di lokasi acara</p>
                        <button
                            type="button"
                            className="jwn-qr-close"
...
export default function DynamicIndex({
    ...
    const handlePointerDown = (clientX, clientY, target) => {
        ...
        if (target && typeof target.closest === 'function') {
            const scrollable = target.closest('.jwn-slide-container');
            if (scrollable) {
    ...
    const handlePointerUp = (clientX, clientY, target) => {
        ...
        if (layoutMode === 'slide-h') {
            ...
            if ((Math.abs(diffY) > Math.abs(diffX)) && (Math.abs(diffY) > threshold || isFastSwipe)) {
                const scrollable = target && typeof target.closest === 'function' ? target.closest('.jwn-slide-container') : null;
                if (scrollable) {
    ...
    const handleWheel = (e) => {
        ...
        const target = e.target.closest('.jwn-slide-container');
    ...
    const renderSectionComponent = (section, idx) => {
        ...
        if (isSlideMode) {
            let slideClass = 'jwn-slide-container';
    ...
    return (
        <ErrorBoundary>
            <div className={`jwn-page ${!showAnimations ? 'jwn-no-animations' : ''}`}>
            ...
            {/* Main Area */}
            <div className="jwn-container">
                <div
                    className={`jwn-main ${isSlideMode ? 'jwn-main--slide' : ''} ${layoutMode === 'slide-h' ? 'jwn-main--slide-h' : ''} ${layoutMode === 'slide-v' ? 'jwn-main--slide-v' : ''}`}
            ...
            {waNumber && isOpened && (
                <div className={`jwn-whatsapp-float ${isBottomMenu ? 'jwn-whatsapp-float--raised' : ''}`}>
                    <a
                        href={`https://wa.me/${waNumber.replace(/[^0-9]/g, '')}`}
                        target="_blank"
                        rel="noopener noreferrer"
                        className="jwn-whatsapp-btn"
