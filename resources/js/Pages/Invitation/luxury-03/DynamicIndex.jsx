    ...
    return (
        <div className={`lx3-cover${isOpened ? ' is-opened' : ''} ${!globalShowPhotos ? 'lx3-no-photo-mode' : ''}`}>
            {globalShowPhotos && coverEmbedId ? (
                <div className="lx3-cover__video-wrapper">
                    <iframe
            ...
            {globalShowPhotos && coverEmbedId ? (
                ...
                <>
                    <PremiumSlideshow
                        ...
                        positionY={invitation?.cover_position_y}
                        zoom={invitation?.cover_zoom}
                        className="lx3-hero__bg is-active absolute inset-0 w-full h-full z-0"
                        imgClassName="absolute inset-0 w-full h-full object-cover"
                    />
                    <div className="absolute inset-0 w-full h-full z-[1]" style={{ backgroundColor: 'rgba(0, 0, 0, 0.4)' }} />
                </>
        ...
        <section id="opening" className="lx3-section lx3-opening">
            <Reveal>
                ...
                {globalShowPhotos && openingEmbedId ? (
                    <div className="lx3-opening__photo-wrapper relative aspect-[16/9]" style={{ margin: '30px auto', maxWidth: '320px', borderRadius: '16px', overflow: 'hidden', border: '4px solid #fff', boxShadow: '0 8px 24px rgba(0,0,0,0.08)' }}>
                        <iframe
                    ...
                    <div className="lx3-opening__photo-wrapper relative aspect-[4/3]" style={{ margin: '30px auto', maxWidth: '320px', borderRadius: '16px', overflow: 'hidden', border: '4px solid #fff', boxShadow: '0 8px 24px rgba(0,0,0,0.08)' }}>
                        <PremiumSlideshow
                        ...
                        <Reveal key={evt.id || idx} variant={isEven ? 'left' : 'right'} className="lx3-event-card-ref">
                            ...
                            <div className="lx3-event-body-ref">
                                ...
                                {list?.filter(evt => evt.show_dress_code).map((evt, idx) => (
                                    <div key={`dc-${idx}`} className="lx3-event-card-ref w-full mt-4" style={{ padding: '24px', backgroundColor: 'rgba(255,255,255,0.02)', border: '1px solid rgba(200, 182, 138, 0.2)' }}>
                                        <DressCodeBlock event={evt} colors={{ primary: '#c8b68a', text: '#333333' }} fonts={{ heading: 'inherit' }} variant="modern" plain={true} />
                                    </div>
                                ))}
            </div>
        </section>
...
function VideoGallerySection({ invitation, locale }) {
    ...
    return (
        <section id="video" className="lx3-section">
            <Reveal>
                <h2 className="lx3-section-title">{locale === 'en' ? 'Video Gallery' : 'Galeri Video'}</h2>
                <p className="lx3-section-subtitle">Our Video Moments</p>
            </Reveal>
                ...
                {videoItems.map((item, idx) => (
                    <Reveal key={idx} variant="zoom" delay={idx * 50} className="w-full">
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                            {videoItems.length > 1 && (
                                <h4 style={{ color: 'var(--lx3-text-dark, #333)', fontSize: '0.9rem', fontWeight: 'bold', margin: '0', letterSpacing: '0.5px' }}>
                                    {item.title}
                                </h4>
                            )}
                            <div 
                                style={{
                                    ...
                                    overflow: 'hidden',
                                    borderRadius: '12px',
                                    border: '1px solid var(--lx3-gold-border, rgba(197, 168, 128, 0.3))',
...
function Navigation({
    ...
    useEffect(() => {
        ...
        const activeEl = document.querySelector(`.lx3-nav-menu button[data-id="${activeMenuId}"]`);
        if (activeEl) {
                        ...
                        {menuItems.map((item) => {
                            ...
                            return (
                                ...
                                >
                                    ...
                                    <span className="lx3-nav-item-text">{item.label}</span>
                                </button>
                            );
                        })}
                    </div>
