...
import moroccanLantern from './asset/moroccan_lantern.png';
...
/* ─── Helper Ornamen Pemisah Bintang Maroko (Rub el Hizb) ─── */
function MoroccanStarDivider() {
    return (
        <div className="mc-divider-decor">
            <div className="mc-divider-star">
                <div className="mc-divider-star-inner" />
            </div>
        </div>
    );
}
...
function Reveal({ children, className = '', variant = 'up', delay = 0 }) {
    ...
    let baseClass = 'mc-reveal--up';
    if (variant === 'zoom') baseClass = 'mc-reveal--zoom';
    else if (variant === 'left') baseClass = 'mc-reveal--left';
    else if (variant === 'right') baseClass = 'mc-reveal--right';
    else if (variant === 'down') baseClass = 'mc-reveal--down';
...
function CoverSection({ invitation, brideGrooms, guest, isOpened, onOpen, themeConfig }) {
    ...
    return (
        <div className={`mc-cover${isOpened ? ' mc-slide--hidden' : ''}`}>
            {/* Majestic Blank Cover as the main background layer */}
            <div className="mc-cover-bg-image absolute inset-0 w-full h-full">
                <img 
                    src={ASSETS.coverBlank} 
                    className="absolute inset-0 w-full h-full object-cover filter brightness-[0.75]" 
                    alt="Background Cover" 
                />
            </div>
            
            <div className="mc-cover-dark-overlay" />
            
            {/* Hanging Glowing lanterns on top left and right */}
            <div className="mc-cover-lanterns-overlay">
                <img src={ASSETS.lantern} className="mc-cover-lantern-left" alt="Lantern Left" />
                <img src={ASSETS.lantern} className="mc-cover-lantern-right" alt="Lantern Right" />
            </div>

            {/* Elegant outer gold border frame */}
            <div className="mc-cover-border-frame" />

            <div className="mc-cover-wrapper">
                {/* 1. Custom Photo Window (Moroccan Arch Shape) */}
                {globalShowPhotos && coverImages.length > 0 ? (
                    <div className="mc-cover-photo-window-container">
                        <div className="mc-cover-photo-window">
                            <PremiumSlideshow
                                images={coverImages}
                            ...
                            />
                            <div className="mc-cover-photo-window-overlay" />
                        </div>
                        {/* Decorative hanging lantern on top of the photo window */}
                        <img src={ASSETS.lantern} className="mc-cover-photo-frame-lantern" alt="Decorative Lantern" />
                    </div>
                ) : (
                    /* 2. Or, if no custom photo, render a beautiful gold Kubah icon as monogram header */
                    <div className="mc-cover-kubah-icon-container">
                        <img src={ASSETS.kubah} className="mc-cover-kubah-icon" alt="Kubah Icon" />
                    </div>
                )}

                {/* Text content blocks */}
                <div className="mc-cover-content-block">
                    <div className="mc-cover-badge">{themeConfig?.labels?.heroBadge || 'THE WEDDING OF'}</div>
                    <h1 className="mc-cover-couple">{coupleName}</h1>
                    
                    {invitation?.countdown_target_date && (
                        <div className="mc-cover-date">
                            {formatDate(invitation.countdown_target_date, invitation?.language)}
                        </div>
                    )}

                    <div className="mc-cover-dear-box">
                        <div className="mc-cover-dear">{isEn ? 'To:' : 'Kepada Yth:'}</div>
                        <div className="mc-cover-guest">{activeGuest.name}</div>
                        <p className="mc-cover-apology">{isEn ? '*We apologize for any spelling errors in names/titles' : '*Mohon maaf apabila terdapat kesalahan penulisan nama/gelar'}</p>
                    </div>

                    <button type="button" onClick={onOpen} id="tombol-buka" className="mc-btn-primary mc-btn-cover-buka">
...
function OpeningSection({ invitation, brideGrooms, language, themeConfig }) {
    ...
    return (
        <section id="opening" className="mc-opening-section-hero">
            {/* Subtle background texture */}
            <div className="mc-opening-bg-texture" />
            
            <div className="mc-opening-hero-block">
                {/* Top hanging lantern decor */}
                <div className="mc-lantern-header-decor">
                    <img src={ASSETS.lantern} className="mc-decor-lantern-left" alt="decor-lantern" />
                    <img src={ASSETS.lantern} className="mc-decor-lantern-right" alt="decor-lantern" />
                </div>

                <Reveal delay={100}>
                    <span className="mc-opening-hero-subtitle">
                        {themeConfig?.labels?.heroBadge || 'THE WEDDING OF'}
                    </span>
                </Reveal>

                {/* Breathtaking Moroccan Palace Photo Window */}
                {globalShowPhotos && openingImages.length > 0 ? (
                    <Reveal className="mc-opening-photo-window-container" variant="zoom" delay={200}>
                        <div className="mc-opening-photo-window">
                            <PremiumSlideshow
                                ...
                                imgClassName="absolute inset-0 w-full h-full object-cover"
                            />
                            <div className="mc-opening-photo-window-overlay" />
                        </div>
                    </Reveal>
                ) : (
                    <Reveal className="mc-opening-photo-window-container" variant="zoom" delay={200}>
                        <div className="mc-opening-kubah-placeholder">
                            <img src={ASSETS.kubah} className="mc-opening-kubah-svg" alt="Moroccan Kubah" />
                        </div>
                    </Reveal>
                )}

                <Reveal delay={300}>
                    <h1 className="mc-opening-hero-couple">{coupleName}</h1>
                </Reveal>
                {formattedDate && (
                    <Reveal delay={500}>
                        <span className="mc-opening-hero-date">
                            {themeConfig?.labels?.heroDateLabel || 'SAVE THE DATE'} | {formattedDate}
                        </span>
                    </Reveal>
                )}

                {/* Countdown Row - Styled as beautiful Moroccan ceramic tiles */}
                {targetDate && (
                    <Reveal className="mc-opening-countdown-row" variant="zoom" delay={700}>
                        <div className="mc-opening-countdown-item">
                            <span className="mc-opening-countdown-value">{String(countdown.days).padStart(2, '0')}</span>
                            <span className="mc-opening-countdown-label">{language === 'en' ? 'Days' : 'Hari'}</span>
                        </div>
                        <div className="mc-opening-countdown-item">
                            <span className="mc-opening-countdown-value">{String(countdown.hours).padStart(2, '0')}</span>
                            <span className="mc-opening-countdown-label">{language === 'en' ? 'Hours' : 'Jam'}</span>
                        </div>
                        <div className="mc-opening-countdown-item">
                            <span className="mc-opening-countdown-value">{String(countdown.minutes).padStart(2, '0')}</span>
                            <span className="mc-opening-countdown-label">{language === 'en' ? 'Mins' : 'Menit'}</span>
                        </div>
                        <div className="mc-opening-countdown-item">
                            <span className="mc-opening-countdown-value">{String(countdown.seconds).padStart(2, '0')}</span>
                            <span className="mc-opening-countdown-label">{language === 'en' ? 'Secs' : 'Detik'}</span>
                        </div>
                    </Reveal>
                )}
            </div>

            <div className="mc-opening-content">
                {/* Moroccan Hanging Lantern Decor (Sederhana & Sesuai) */}
                <Reveal className="mc-opening-lantern-divider" variant="zoom">
                    <img src={ASSETS.lantern} className="mc-opening-lantern-divider-img" alt="Moroccan Lantern" />
                </Reveal>

                {/* Text block with clean background and high contrast */}
                <Reveal className="mc-opening-text-block">
                    <h2 className="mc-opening-basmalah">{invitation?.religion === 'islam' ? 'Bismillahirrahmanirrahim' : (language === 'en' ? 'Welcome' : 'Selamat Datang')}</h2>
                    <div className="mc-opening-salut">
                ...
                {invitation?.opening_ayat && (
                    <Reveal className="mc-quote-box" variant="zoom" delay={200}>
...
function CoupleSection({ brideGrooms, language, id, themeConfig }) {
        ...
        return (
            <div className="mc-couple-card">
                {globalShowPhotos && m.photo ? (
                    ...
                    <Reveal className="mc-profile-frame-wrap" variant="zoom">
                        <div className="mc-profile-arch-frame mc-profile-monogram-frame">
                            <div className="mc-profile-monogram-initial">
                ...
                <Reveal variant="up" delay={100}>
                    <h3 className="mc-profile-name">{m.full_name}</h3>
                </Reveal>
                <Reveal variant="up" delay={200}>
                    <div className="mc-profile-child-order">
                        {translateChildOrder(m.child_order, m.gender, isEn)}
                    </div>
                </Reveal>
                <Reveal variant="up" delay={300}>
                    <div className="mc-profile-parents">
                ...
                {(m.instagram || m.instagram_username) && (
                    <Reveal variant="zoom" delay={400}>
                        <a
                            ...
                            className="mc-profile-instagram"
                        >
    ...
    return (
        <section id={id || "couple"} className="mc-section">
            <Reveal>
                <h4 className="mc-section-subtitle">{coupleSubtitle}</h4>
                <h2 className="mc-section-title">{coupleTitle}</h2>
            </Reveal>

            {isSingleHost ? (
                renderMempelai(couples[0] || {}, 'host')
            ) : (
                <div className="mc-couples-grid">
                    {renderMempelai(groom, 'groom')}
                    {groom.full_name && bride.full_name && (
                        <div className="mc-couple-ampersand">&amp;</div>
                    )}
...
function EventSection({ events, invitation, sections, language, themeConfig }) {
    ...
    return (
        <section id="event" className="mc-section">
            <Reveal>
                <h4 className="mc-section-subtitle">{eventSubtitle}</h4>
                <h2 className="mc-section-title">{eventTitle}</h2>
            </Reveal>

            {/* Countdown widget */}
            {showCountdownInEvent && invitation?.countdown_target_date && (
                <Reveal className="mc-countdown-row" variant="zoom">
                    <div className="mc-countdown-item">
                        <span className="mc-countdown-value">{String(countdown.days).padStart(2, '0')}</span>
                        <span className="mc-countdown-label">{language === 'en' ? 'Days' : 'Hari'}</span>
                    </div>
                    <div className="mc-countdown-item">
                        <span className="mc-countdown-value">{String(countdown.hours).padStart(2, '0')}</span>
                        <span className="mc-countdown-label">{language === 'en' ? 'Hours' : 'Jam'}</span>
                    </div>
                    <div className="mc-countdown-item">
                        <span className="mc-countdown-value">{String(countdown.minutes).padStart(2, '0')}</span>
                        <span className="mc-countdown-label">{language === 'en' ? 'Mins' : 'Menit'}</span>
                    </div>
                    <div className="mc-countdown-item">
                        <span className="mc-countdown-value">{String(countdown.seconds).padStart(2, '0')}</span>
                        <span className="mc-countdown-label">{language === 'en' ? 'Secs' : 'Detik'}</span>
                    </div>
                </Reveal>
            )}
            ...
            <div className="mc-events-list">
                {listEvents.map((ev, idx) => {
                    return (
                        <Reveal className="mc-event-card" variant={idx % 2 === 0 ? 'left' : 'right'} key={idx}>
                            <div className="mc-event-card-topper">
                                <img src={ASSETS.lantern} className="mc-event-card-lantern-topper" alt="Hanging Lantern" />
                            </div>
                            <div className="mc-event-card-header">
                                <div className="mc-event-name">{ev.event_name}</div>
                            </div>
                            
                            <div className="mc-event-date-box">
                                <div className="mc-event-date">{formatDate(ev.event_date, language)}</div>
                                <div className="mc-event-time">
                                    {ev.start_time} - {ev.end_time || 'Selesai'} {ev.timezone || 'WIB'}
                                </div>
                            </div>

                            <div className="mc-event-venue">{ev.venue_name}</div>
                            <p className="mc-event-address">{ev.venue_address}</p>

                            {ev.gmaps_link && (
                                <div className="mc-event-actions">
                                    <a href={ev.gmaps_link} target="_blank" rel="noopener noreferrer" className="mc-btn-secondary">
...
function LiveStreamingSection({ events, invitation, language, themeConfig }) {
    ...
    return (
        <section id="livestream" className="mc-section mc-livestream-section">
            <Reveal>
                <h4 className="mc-section-subtitle">{streamSubtitle}</h4>
                <h2 className="mc-section-title">{streamTitle}</h2>
            </Reveal>

            <Reveal className="mc-moroccan-arch-container">
                {/* Decorative hanging lantern on top of the arch container (Sederhana & Sesuai) */}
                <div className="mc-arch-topper">
                    <img src={ASSETS.lantern} className="mc-arch-topper-lantern" alt="Hanging Lantern" />
                </div>
                <div className="mc-arch-inner-content">
                    <h3 className="mc-livestream-date">
                        {formatDate(primaryEvent?.event_date, language)}
                    </h3>
                    <p className="mc-livestream-time">
                        {primaryEvent?.start_time} - {primaryEvent?.end_time === '23:59:00' ? 'Selesai' : primaryEvent?.end_time} {primaryEvent?.timezone || 'WIB'}
                    </p>
                    <p className="mc-livestream-text">
                        {streamDesc}
                    </p>
                    <div className="mc-livestream-buttons">
...
function TimelineCard({ story, language }) {
    ...
    return (
        <div ref={ref} className={`mc-timeline-card ${isActive ? 'is-active' : ''}`}>
            <div className="mc-timeline-dot">
                <img src={ASSETS.lantern} className="mc-timeline-lantern-icon" alt="lantern" />
            </div>
            {story.story_date && (
                <div className="mc-timeline-date">{formatDate(story.story_date, language)}</div>
            )}
            <h3 className="mc-timeline-title">{story.title}</h3>
            {story.description && (
                <p className="mc-timeline-desc">{story.description}</p>
            )}
        </div>
    );
}
...
function LoveStorySection({ loveStories, language, themeConfig }) {
    ...
    return (
        <section id="love_story" className="mc-section">
            <Reveal>
                <h4 className="mc-section-subtitle">{storySubtitle}</h4>
                <h2 className="mc-section-title">{storyTitle}</h2>
            </Reveal>
...
function GallerySection({ galleries, language }) {
    ...
    return (
        <section id="gallery" className="mc-section">
            <Reveal>
                <h4 className="mc-section-subtitle">Momen Bahagia</h4>
                <h2 className="mc-section-title">{language === 'en' ? 'GALLERY' : 'GALERI FOTO'}</h2>
            </Reveal>
            ...
            <div className="mc-gallery-grid">
                {listGalleries.map((g, idx) => {
                    ...
                    return (
                        <Reveal className="mc-gallery-item" variant="zoom" delay={idx * 100} key={idx}>
                            <div className="mc-gallery-frame" onClick={() => openLightbox(idx)}>
                                <img src={finalImg} className="mc-gallery-photo" alt={g.caption || 'Galeri'} />
                                <div className="mc-gallery-hover-overlay">
            ...
            {/* Lightbox Modal */}
            {lightbox.open && (
                <div className="mc-lightbox" onClick={closeLightbox}>
                    <button type="button" className="mc-lightbox-close" onClick={closeLightbox}>&times;</button>
                    <button type="button" className="mc-lightbox-arrow mc-lightbox-arrow--left" onClick={prevImage}>
                    ...
                    <div className="mc-lightbox-content" onClick={e => e.stopPropagation()}>
                        ...
                        {listGalleries[lightbox.index]?.caption && (
                            <p className="mc-lightbox-caption">{listGalleries[lightbox.index].caption}</p>
                        )}
                    </div>

                    <button type="button" className="mc-lightbox-arrow mc-lightbox-arrow--right" onClick={nextImage}>
...
function BankSection({ bankAccounts, language }) {
    ...
    return (
        <section id="bank" className="mc-section">
            <Reveal>
                <h4 className="mc-section-subtitle">Kirim Kado</h4>
                <h2 className="mc-section-title">{language === 'en' ? 'DIGITAL ENVELOPE' : 'AMPLOP DIGITAL'}</h2>
            </Reveal>

            <Reveal className="mc-bank-desc-wrap">
                <p className="mc-bank-desc">
                    {language === 'en' 
            ...
            <div className="mc-bank-cards-list">
                {listAccounts.map((acc, idx) => {
                    ...
                    return (
                        <Reveal className="mc-credit-card" variant="zoom" delay={idx * 150} key={idx}>
                            <div className="mc-card-inner">
                                <div className="mc-card-shine" />
                                <div className="mc-card-header">
                                    <span className="mc-card-label">Digital Envelope</span>
                                    {isBca && <img src={ASSETS.bca} className="mc-card-bank-logo" alt="BCA" />}
                                    {isDana && <img src={ASSETS.dana} className="mc-card-bank-logo mc-card-bank-logo--dana" alt="DANA" />}
                                    {!isBca && !isDana && <span className="mc-card-bank-name">{acc.bank_name}</span>}
                                </div>

                                <div className="mc-card-chip-row">
                                    <img src={ASSETS.chip} className="mc-card-chip" alt="ATM Chip" />
                                    <img src={ASSETS.lantern} className="mc-card-lantern-monogram-decor" alt="Lantern" />
                                </div>

                                <div className="mc-card-number">{acc.account_number}</div>

                                <div className="mc-card-footer">
                                    <div className="mc-card-holder">
                                        <span className="mc-holder-label">{language === 'en' ? 'ON BEHALF OF' : 'ATAS NAMA'}</span>
                                        <span className="mc-holder-name">{acc.account_name}</span>
                                    </div>
                                    <button 
                                        type="button" 
                                        onClick={() => handleCopy(acc.account_number, idx)}
                                        className="mc-card-btn-copy"
                                    >
                                        {copiedIdx === idx ? (
                                            <>
...
function UnifiedRsvpWishes({ invitation, wishes, guest, language, enableRsvp, enableWishes }) {
    ...
    return (
        <section id="rsvp" className="mc-section">
            <Reveal>
                <h4 className="mc-section-subtitle">{isEn ? 'Confirmation' : 'Konfirmasi Kehadiran'}</h4>
                <h2 className="mc-section-title">{sectionTitle}</h2>
            </Reveal>

            <Reveal className="mc-rsvp-box-container">
                <div className="mc-rsvp-box-topper">
                    <img src={ASSETS.lantern} className="mc-rsvp-box-lantern-topper" alt="Hanging Lantern" />
                </div>
                <form onSubmit={handleSubmit} className="mc-rsvp-form">
                    {/* Name Input */}
                    <div className="mc-form-group">
                        <label className="mc-form-label" htmlFor="rsvp_name">{isEn ? 'Your Name' : 'Nama Lengkap'}</label>
                        <input
                            type="text"
                            id="rsvp_name"
                            value={sharedName}
                    ...
                    {enableRsvp && (
                        <div className="mc-form-group">
                            <label className="mc-form-label">{isEn ? 'Attendance status' : 'Konfirmasi Kehadiran'}</label>
                            <div className="mc-rsvp-buttons-row">
                                {['hadir', 'tidak_hadir', 'belum_pasti'].map(opt => {
                                    ...
                                    return (
                                        <button
                                            ...
                                            className={`mc-rsvp-btn-option ${active ? 'is-active' : ''}`}
                                        >
                    ...
                    {enableRsvp && attendance === 'hadir' && (
                        <Reveal className="mc-form-group" variant="down">
                            <label className="mc-form-label" htmlFor="number_of_guests">
                    ...
                    {enableWishes && (
                        <div className="mc-form-group">
                            ...
                            <WishesEmojiPicker
                                ...
                                className="mc-form-input mc-form-input--textarea"
                                rows={4}
                                required={!enableRsvp}
                            />
                                </WishesEmojiPicker>
                    ...
                    <button type="submit" disabled={isSubmitting} className="mc-btn-primary mc-btn-full">
                        {isSubmitting ? (isEn ? 'Sending...' : 'Mengirim...') : (isEn ? 'Send' : 'Kirim')}
                    </button>

                    {success && (
                        <p style={{ color: 'var(--mc-accent)', marginTop: '8px', fontSize: '12px', textAlign: 'center', fontWeight: 'bold' }}>
            ...
            {enableWishes && recentWishes.length > 0 && (
                <div className="mc-wishes-list-container" style={{ marginTop: '24px' }}>
                    {recentWishes.map((w, idx) => (
                        <Reveal className="mc-wish-bubble" variant="up" delay={idx * 50} key={idx}>
                            <div className="mc-wish-header">
                                <span className="mc-wish-sender">{w.sender_name || w.name}</span>
                                {w.attendance && (
                                    <span style={{ fontSize: '10px', color: 'var(--mc-primary)', border: '1px solid var(--mc-primary)', padding: '2px 8px', borderRadius: '12px' }}>
                                        {w.attendance === 'hadir' ? (isEn ? 'Attending' : 'Hadir') : (isEn ? 'Absent' : 'Tidak Hadir')}
                                    </span>
                                )}
                            </div>
                            <p className="mc-wish-message">{w.message}</p>
                        </Reveal>
                    ))}
                </div>
            )}
...
function ClosingSection({ invitation, brideGrooms, language, themeConfig }) {
    ...
    return (
        <section id="closing" className="mc-section mc-closing-section">
            {/* Top decorative hanging lantern (Sederhana & Sesuai) */}
            <div className="mc-closing-lantern-top">
                <img src={ASSETS.lantern} className="mc-closing-lantern-img" alt="Lantern Decor" />
            </div>

            <div className="mc-closing-body">
                <Reveal className="mc-closing-text-block">
                    <h2 className="mc-closing-title">{invitation?.closing_title || 'THANK YOU'}</h2>
                    <p className="mc-closing-text">
                        {themeConfig?.labels?.closingQuote}
                    </p>
                </Reveal>

                <Reveal delay={200} className="mc-closing-signature">
                    <p className="mc-signature-label">{themeConfig?.labels?.signatureTitle || 'Kami yang berbahagia,'}</p>
                    <h3 className="mc-signature-couple">{coupleName}</h3>
                    
                    <div className="mc-signature-parents" style={{ fontSize: '12px', color: 'var(--mc-text-muted)', lineHeight: '1.8', marginBottom: '20px' }}>
                        {hasGroomParents && (
                            <div>
                                {isEn
                    ...
                    {invitation?.turut_mengundang_text && (
                        <div className="mc-signature-turut">
                            <span className="mc-turut-label">{themeConfig?.labels?.signatureFamilies || 'Keluarga Besar:'}</span>
                            <p className="mc-turut-text" style={{ whiteSpace: 'pre-line' }}>{invitation.turut_mengundang_text}</p>
                        </div>
                    )}
                </Reveal>
            </div>

            <Reveal className="mc-closing-watermark">
                <p>Made with ❤️ by {brandName}</p>
            </Reveal>

            <div className="mc-closing-lantern-decor">
                <img src={ASSETS.lantern} className="mc-decor-lantern-bottom" alt="decor-lantern" />
            </div>
        </section>
    );
}
...
function NavigationMenu({
    ...
    useEffect(() => {
        ...
        const activeEl = document.querySelector(`.mc-nav-menu button[data-id="${activeMenuId}"]`);
        if (activeEl) {
    ...
    return (
        <>
            {/* Dock Menu Bawah */}
            <div className="mc-nav-menu">
                <div className="mc-nav-menu__inner--row">
                    {menuItems.map((item) => {
                        ...
                        return (
                            <button
                                ...
                                className={`mc-nav-menu__item ${isActive ? 'active' : ''}`}
                                title={item.label}
                            >
                                <i className={item.icon} />
                                <span className="mc-nav-item-text">{item.label}</span>
                            </button>
                        );
                    })}
                </div>
            ...
            <div className="mc-floating-btns">
                {enableQr && guest && (
                    <button
                        type="button"
                ...
                <button
                    ...
                    style={isFullscreen ? { backgroundColor: 'var(--mc-accent, #E9C46A)', color: 'var(--mc-bg)', boxShadow: '0 0 12px var(--mc-accent)' } : {}}
                ...
                {invitation?.enable_auto_scroll !== false && (
                    <button
                        ...
                        style={autoScrollEnabled ? { backgroundColor: 'var(--mc-accent, #E9C46A)', color: 'var(--mc-bg)', boxShadow: '0 0 12px var(--mc-accent)' } : {}}
...
export default function DynamicIndex({ invitation, sections, brideGrooms, events, galleries, loveStories, bankAccounts, wishes, guest, isDemo = false }) {
    ...
    const handlePointerDown = (clientX, clientY, target) => {
        ...
        if (target && typeof target.closest === 'function') {
            const scrollable = target.closest('.mc-slide-container');
            if (scrollable) {
    ...
    const handlePointerUp = (clientX, clientY, target) => {
        ...
        if (layoutMode === 'slide-h') {
            ...
            if ((Math.abs(diffY) > Math.abs(diffX)) && (Math.abs(diffY) > threshold || isFastSwipe)) {
                const scrollable = target && typeof target.closest === 'function' ? target.closest('.mc-slide-container') : null;
                if (scrollable) {
    ...
    const handleWheel = (e) => {
        ...
        const target = e.target.closest('.mc-slide-container');
    ...
    useEffect(() => {
        ...
        if (isSlideMode) {
            timer = setInterval(() => {
                const activeEl = containerRef.current?.querySelector('.mc-slide-container.is-active');
                if (activeEl) {
    ...
    return (
        <ErrorBoundary>
            ...
            <div className={`mc-page ${!globalShowPhotos ? 'mc-no-photo-mode' : ''} ${!globalShowAnimations ? 'mc-no-animations' : ''}`} ref={containerRef}>
                ...
                {isOpened && (
                    <div className="mc-container">
                        ...
                        <div className="mc-container-bg" style={{
                            backgroundImage: `linear-gradient(to bottom, rgba(18, 30, 42, 0.93), rgba(18, 30, 42, 0.93)), url(${ASSETS.coverBlank})`,
                        ...
                        <div 
                            className={`${isSlideMode ? 'mc-main-slide' : 'mc-scroll-container'} ${layoutMode === 'slide-h' ? 'mc-main--slide-h' : ''} ${layoutMode === 'slide-v' ? 'mc-main--slide-v' : ''}`}
                            ...
                            <>
                                {activeSlideIdx > 0 && (
                                    <button 
                                        type="button"
                                        onClick={goPrev}
                                        className="mc-slider-arrow mc-slider-arrow--left"
                                ...
                                {activeSlideIdx < activeSectionsWithoutCover.length - 1 && (
                                    <button 
                                        type="button"
                                        onClick={goNext}
                                        className="mc-slider-arrow mc-slider-arrow--right"
                                        style={{ color: 'var(--mc-primary)' }}
                                    >
                                        <i className="fas fa-chevron-right" />
                                    </button>
                                )}
                                <div className="mc-slider-dots-row">
                                    {activeSectionsWithoutCover.map((_, i) => (
                                        <button 
                                            type="button"
                                            key={i} 
                                            onClick={() => setActiveSlideIdx(i)}
                                            className={`mc-slider-dot ${i === activeSlideIdx ? 'is-active' : ''}`}
                                        />
                                    ))}
                                </div>
                            </>
                        ...
                        {enableQr && showQr && guest && (
                            <div className="mc-qr-modal-overlay" onClick={() => setShowQr(false)}>
                                <div className="mc-qr-modal-body" onClick={e => e.stopPropagation()}>
                                    <h3 className="mc-qr-modal-title">QR CHECK-IN</h3>
                                    <p className="mc-qr-modal-subtitle">{guest.name}</p>
                                    <div className="mc-qr-modal-canvas-wrap">
                                        <img 
                                            ...
                                            alt="QR" 
                                            className="mc-qr-canvas" 
                                        />
                                    </div>
                                    <p className="mc-qr-modal-tip">Tunjukkan QR Code ini kepada petugas penerima tamu</p>
                                    <button type="button" className="mc-btn-primary" onClick={() => setShowQr(false)}>
                                        Tutup
                                    </button>
                                </div>
                            </div>
