...
function CoverSection({ invitation, brideGrooms, guest, isOpened, onOpen, showPhotos }) {
    ...
    return (
        <div className={`sp08-cover ${isOpened ? 'sp08-is-opened' : ''}`}>
            {/* Golden Frame Decorative Overlays */}
            <div className="sp08-cover-decor-inner-border" />
            <div className="sp08-cover-decor-top-left" />
            <div className="sp08-cover-decor-top-right" />
            <div className="sp08-cover-decor-bottom-left" />
            <div className="sp08-cover-decor-bottom-right" />
            ...
            <div className="sp08-cover__content">
                ...
                <div className="sp08-cover-guest-box my-4">
                    <p className="text-[10px] text-[var(--sp08-text-muted)] font-semibold tracking-widest mb-2 select-none">
...
function ClosingSection({ invitation, brideGrooms, locale }) {
    ...
    return (
        <div className="sp08-closing-section">
            <Reveal variant="zoom">
                <FlowerSwirl title={formattedTitle} subtitle={isEn ? 'Thank You for Blessing Us' : 'Doa Restu Anda Adalah Berkat Kami'} />
            </Reveal>
...
export default function DynamicIndex({ invitation, sections, brideGrooms, events, galleries, loveStories, bankAccounts, guest, wishes }) {
    ...
    useEffect(() => {
        if (!isOpened) return;
        const navEl = document.querySelector('.sp08-nav-menu');
        ...
        <ErrorBoundary>
            ...
            <div className={`sp08-wrapper select-text ${!showAnimations ? 'theme-no-animations' : ''}`}>
                ...
                {isOpened && (
                    <div className="sp08-floating-controls">
                        ...
                        <button 
                            ...
                            className={`sp08-btn-floating ${autoScroll ? 'is-active' : ''}`}
                            ...
                            {key === 'opening' && (
                                <div>
                                    ...
                                    <Reveal delay={200} className="w-full text-center my-6 px-4">
                                        <p className="text-xs sm:text-sm leading-relaxed opacity-95 text-[var(--sp08-text-muted)] font-serif italic max-w-sm mx-auto select-none">
                ...
                {isOpened && resolvedSections.length > 0 && (
                    <nav className="sp08-nav-menu">
                        <div className="sp08-nav-menu__inner--row">
                            {resolvedSections.map(s => {
                                ...
                                return (
                                    <button
                                        ...
                                        className={`sp08-nav-menu__item ${isActive ? 'active' : ''}`}
                                        title={s.section_name}
                                    >
                                        <i className={icon} />
                                        <span className="sp08-nav-item-text">{labelText}</span>
                                    </button>
                                );
                            })}
                        </div>
