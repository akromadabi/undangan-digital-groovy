...
/* ═══════════════════════════════════════
   ERROR BOUNDARY
   ═══════════════════════════════════════ */
class ErrorBoundaryLuxury4 extends React.Component {
    constructor(props) { super(props); this.state = { hasError: false, error: null }; }
    static getDerivedStateFromError(e) { return { hasError: true, error: e }; }
    render() {
        if (this.state.hasError) return (
            <div style={{ padding: 20, color: '#fff', background: '#111', minHeight: '100vh', fontFamily: 'monospace' }}>
                <h2 style={{ color: '#ff4d4d' }}>Terjadi kesalahan pada tema Luxury 04.</h2>
...
function Reveal({ children, className = '', variant = 'up', delay = 0 }) {
    ...
    let baseClass = 'lx4-reveal--up';
    if (variant === 'zoom') baseClass = 'lx4-reveal--zoom';
    else if (variant === 'left') baseClass = 'lx4-reveal--left';
    else if (variant === 'right') baseClass = 'lx4-reveal--right';
    else if (variant === 'down') baseClass = 'lx4-reveal--down';
    ...
    return (
        <div className={`lx4-cover${isOpened ? ' is-opened' : ''} ${!globalShowPhotos ? 'lx4-no-photo-mode' : ''}`}>
            ...
            <div className="lx4-cover__content">
                ...
                <div className="lx4-cover__circle-logo">
                    ...
                    <div className="lx4-cover__monogram">{initials}</div>
                </div>

                <h1 className="lx4-cover__couple">{coupleName}</h1>
                <p className="lx4-cover__dear">Dear:</p>
                <div className="lx4-cover__guest">{guestName}</div>
                <p className="lx4-cover__apology">{t('invitation.dear_guest_desc')}</p>

                <button type="button" onClick={onOpen} id="tombol-buka" className="lx4-cover__btn">
 ...
 {list?.filter(p => p.show_dress_code).map((p, idx) => (
    ...
    return (
        <section id="hero" className="lx4-hero">
            ...
            <div className="lx4-hero__overlay" />
            <div className="lx4-hero__content">
                <Reveal delay={200}>
                    <span className="lx4-hero__subtitle">{t('invitation.wedding_of').toUpperCase()}</span>
                </Reveal>
                <Reveal delay={400}>
                    <h1 className="lx4-hero__title">{coupleName}</h1>
                </Reveal>
                <Reveal delay={600}>
                    <span className="lx4-hero__date">{t('invitation.save_the_date').toUpperCase()} | {formattedDate}</span>
                </Reveal>
                
                <Reveal delay={800}>
                    <div className="lx4-hero__countdown">
                        <div className="lx4-hero__countdown-item">
                            <span className="lx4-hero__countdown-val">{pad2(timeLeft.d)}</span>
                            <span className="lx4-hero__countdown-label">{t('invitation.days')}</span>
                        </div>
                        <div className="lx4-hero__countdown-item">
                            <span className="lx4-hero__countdown-val">{pad2(timeLeft.h)}</span>
                            <span className="lx4-hero__countdown-label">{t('invitation.hours')}</span>
                        </div>
                        <div className="lx4-hero__countdown-item">
                            <span className="lx4-hero__countdown-val">{pad2(timeLeft.m)}</span>
                            <span className="lx4-hero__countdown-label">{t('invitation.minutes')}</span>
                        </div>
                        <div className="lx4-hero__countdown-item">
                            <span className="lx4-hero__countdown-val">{pad2(timeLeft.s)}</span>
                            <span className="lx4-hero__countdown-label">{t('invitation.seconds')}</span>
                        </div>
                    </div>
                </Reveal>
                
                <div className={`lx4-hero__arrow ${isHorizontal ? 'lx4-hero__arrow--horizontal' : 'lx4-hero__arrow--vertical'}`}>
...
function OpeningSection({ invitation, brideGrooms, openingEmbedId }) {
    ...
    return (
        <section id="opening" className="lx4-section lx4-opening">
            <Reveal>
                {isWeddingOf ? (
                    <div className="lx4-opening__header">
                        <span className="lx4-opening__subtitle">{t('invitation.wedding_of').toUpperCase()}</span>
                        {first && second && (
                            <div className="lx4-opening__initials">
                                <span className="lx4-opening__initials-letter">{first}</span>
                                <span className="lx4-opening__initials-amp">&amp;</span>
                                <span className="lx4-opening__initials-letter">{second}</span>
                            </div>
                        )}
                    </div>
                ) : (
                    <h2 className="lx4-opening__title">{invitation?.opening_title || 'Maha Suci Allah'}</h2>
                )}
                
                {invitation?.opening_ayat && (
                    <div className="lx4-opening__quote-container">
                        <p className="lx4-opening__ayat">&ldquo;{invitation.opening_ayat}&rdquo;</p>
                        {invitation?.opening_ayat_source && (
                            <p className="lx4-opening__source">&mdash; {invitation.opening_ayat_source}</p>
                        )}
                    </div>
                )}
...
function BrideGroomSection({ brideGrooms }) {
    ...
    return (
        <section id="bride_groom" className="lx4-section lx4-couple">
            <Reveal variant="up">
                <h2 className="lx4-section-title">{t('invitation.mempelai')}</h2>
                <p className="lx4-section-subtitle">Groom &amp; Bride</p>
            </Reveal>
            ...
            <Reveal className="lx4-mempelai-card" variant="left">
                ...
                <div className="lx4-mempelai-details">
                    <h3 className="lx4-mempelai-name">{groom.full_name || 'Nama Lengkap Pria'}</h3>
                    <p className="lx4-mempelai-parent-label">
                        {translateChildOrder(groom.child_order, 'pria')}
                    </p>
                    <p className="lx4-mempelai-parents">
            ...
            <Reveal variant="zoom">
                <div className="lx4-mempelai-amp">&amp;</div>
            </Reveal>
            ...
            <Reveal className="lx4-mempelai-card" variant="right">
                ...
                <div className="lx4-mempelai-details">
                    <h3 className="lx4-mempelai-name">{bride.full_name || 'Nama Lengkap Wanita'}</h3>
                    <p className="lx4-mempelai-parent-label">
        ...
        <div className="lx4-countdown-wrapper" style={{ position: 'relative', overflow: 'hidden', padding: '60px 24px' }}>
            {globalShowPhotos && bgImg && (
                <>
                    ...
                    <div className="lx4-section-overlay" />
                </>
            )}
            <div style={{ position: 'relative', zIndex: 2 }}>
                <Reveal>
                    <h2 className="lx4-section-title">{t('invitation.save_the_date')}</h2>
                    <p className="lx4-section-subtitle">{t('invitation.save_the_date') === 'Save The Date' ? 'Counting Down' : 'Menghitung Hari'}</p>
                </Reveal>

                <Reveal variant="zoom">
                    <div className="lx4-countdown">
                        {/* Days */}
                        <div className="lx4-countdown__item">
                            <div className="lx4-countdown__value">
                                {dDigits.map((digit, idx) => (
                                    <span key={`d-${idx}`} className="lx4-countdown__digit">{digit}</span>
                                ))}
                            </div>
                            <span className="lx4-countdown__label">{t('invitation.days')}</span>
                        </div>

                        <span className="lx4-countdown__colon">:</span>

                        {/* Hours */}
                        <div className="lx4-countdown__item">
                            <div className="lx4-countdown__value">
                                {hDigits.map((digit, idx) => (
                                    <span key={`h-${idx}`} className="lx4-countdown__digit">{digit}</span>
                                ))}
                            </div>
                            <span className="lx4-countdown__label">{t('invitation.hours')}</span>
                        </div>
                        ...
                        <div className="lx4-countdown__item">
                            <div className="lx4-countdown__value">
                                {mDigits.map((digit, idx) => (
                                    <span key={`m-${idx}`} className="lx4-countdown__digit">{digit}</span>
                                ))}
                            </div>
                            <span className="lx4-countdown__label">{t('invitation.minutes')}</span>
                        </div>
                        ...
                        <div className="lx4-countdown__item">
                            <div className="lx4-countdown__value">
                                {sDigits.map((digit, idx) => (
                                    <span key={`s-${idx}`} className="lx4-countdown__digit">{digit}</span>
                                ))}
                            </div>
                            <span className="lx4-countdown__label">{t('invitation.seconds')}</span>
                        </div>
                    </div>
                </Reveal>
            </div>
...
function EventSection({ events, galleries, showCountdown }) {
    ...
    return (
        <section id="event" className="lx4-section" style={{ padding: showCountdown ? '0 0 60px 0' : '60px 24px' }}>
            {showCountdown && (
            ...
            <div style={{ padding: showCountdown ? '60px 24px 0 24px' : '0' }}>
                <Reveal>
                    <h2 className="lx4-section-title">{t('nav.acara')}</h2>
                    <p className="lx4-section-subtitle">{t('invitation.wedding_events')}</p>
                </Reveal>
                ...
                {list.map((evt, idx) => {
                    ...
                    return (
                        <Reveal key={evt.id || idx} variant={isEven ? 'left' : 'right'} className="lx4-event-card-ref">
                            {globalShowPhotos && eventImg && (
                                <div className="lx4-event-banner-ref">
                                    <img src={eventImg} alt={evt.event_name} />
                                </div>
                            )}
                            <div className="lx4-event-body-ref">
                                {/* Left column: Dark gray vertical name */}
                                <div className="lx4-event-left-ref">
                                    <div className="lx4-event-vertical-title">{eventDisplayName}</div>
                                </div>
                                {/* Right column: details */}
                                <div className="lx4-event-right-ref">
                                    {/* Date row */}
                                    <div className="lx4-event-date-row-ref">
                                        <span className="lx4-event-date-num-ref">{dayNum}</span>
                                        <div className="lx4-event-date-col-ref">
                                            <span className="lx4-event-date-day-ref">{dayName}</span>
                                            <span className="lx4-event-date-month-ref">{monthName}</span>
                                            <span className="lx4-event-date-year-ref">{year}</span>
                                        </div>
                                    </div>
                                    <hr className="lx4-event-divider-ref" />
                                    {/* Time */}
                                    <p className="lx4-event-time-ref">
                                    ...
                                    {/* Location */}
                                    <div className="lx4-event-location-ref">
                                        <h4 className="lx4-event-location-title-ref">{t('invitation.save_the_date') === 'Save The Date' ? 'Event Location' : 'Lokasi Acara'}</h4>
                                        <p className="lx4-event-venue-ref">{t('invitation.save_the_date') === 'Save The Date' ? 'Venue' : 'Tempat'} : <strong>{evt.venue_name}</strong></p>
                                        <p className="lx4-event-address-ref">{evt.venue_address}</p>
                                    </div>
                                    {/* Maps Button */}
                                    {evt.gmaps_link && (
                                        <a
...
function LoveStorySection({ loveStories }) {
    ...
    return (
        <section id="love_story" className="lx4-section">
            <Reveal>
                <h2 className="lx4-section-title">{t('invitation.love_story')}</h2>
                <p className="lx4-section-subtitle">Our Love Story</p>
            </Reveal>

            <div className="lx4-story">
                {list.map((story, idx) => {
                    const isEven = idx % 2 === 0;
                    return (
...
function GallerySection({ galleries }) {
    ...
    return (
        <section id="gallery" className="lx4-section">
            <Reveal>
                <h2 className="lx4-section-title">{t('invitation.gallery')}</h2>
                <p className="lx4-section-subtitle">Our Moments</p>
            </Reveal>

            <div className="lx4-gallery-grid">
                {list.map((photo, idx) => {
                    ...
                    return (
                        <Reveal
                            key={photo.id || idx}
                            variant="zoom"
                            className="lx4-gallery-grid__item"
                        >
                            <img
                                src={imgUrl}
            ...
            {activeImg && (
                <div className="lx4-lightbox" onClick={() => setActiveImg(null)}>
                    <button
                        type="button"
                        className="lx4-lightbox__close"
...
function BankSection({ bankAccounts, invitation }) {
    ...
    return (
        <section id="bank" className="lx4-section">
            <Reveal>
                <h2 className="lx4-section-title">{t('nav.bank')}</h2>
...
function LiveStreamingSection({ events, invitation }) {
    ...
    return (
        <section id="livestream" className="lx4-section lx4-section-livestream" style={{ padding: '60px 24px' }}>
            <Reveal>
                <h2 className="lx4-section-title">LIVE STREAMING</h2>
                <p className="lx4-section-subtitle">{isEn ? 'Virtual Wedding Celebration' : 'Prosesi Pernikahan Virtual'}</p>
            </Reveal>

            <Reveal className="lx4-card" style={{ maxWidth: '480px', margin: '0 auto', textAlign: 'center', padding: '30px 20px' }}>
                <h3 className="lx4-event-card__name" style={{ color: '#d8c3b0', fontSize: '18px', marginBottom: 15, fontFamily: 'var(--lx4-font-serif)' }}>
                ...
                <p style={{ fontSize: '12px', color: 'rgba(255, 255, 255, 0.7)', lineHeight: '1.6', marginBottom: '25px' }}>
                    {isEn 
...
function UnifiedRsvpWishes({ invitation, wishes, guest, enableRsvp, enableWishes }) {
    ...
    return (
        <section id="rsvp" className="lx4-section">
            <Reveal>
                <h2 className="lx4-section-title">{sectionTitle}</h2>
                <p className="lx4-section-subtitle">
                    {isEn
            ...
            <Reveal>
                <div id="rsvp_container" className="lx4-card" style={{ maxWidth: '500px', margin: '0 auto' }}>
                    <form onSubmit={handleSubmit} className="lx4-rsvp-form">
                        {/* Nama */}
                        <div className="lx4-form-group">
                        ...
                        {success && (
                            <p style={{ color: 'var(--lx4-gold)', marginTop: '12px', fontSize: '13px', textAlign: 'center' }}>
                    ...
                    {enableWishes && recentWishes.length > 0 && (
                        <div className="lx4-wishes-list-wrapper" style={{ marginTop: '20px' }}>
                            <div className="lx4-wishes-list" style={{ maxHeight: '280px', overflowY: 'auto' }}>
                                {recentWishes.map((w, idx) => (
                                    <div key={w.id || idx} className="lx4-wish-bubble">
                                        <div className="lx4-wish-bubble__header">
                                            <span className="lx4-wish-bubble__sender">{w.sender_name || w.name}</span>
                                            {w.attendance && (
                                                <span className={`lx4-wish-bubble__badge ${w.attendance === 'hadir' ? 'lx4-wish-bubble__badge--hadir' : 'lx4-wish-bubble__badge--tidak'}`}>
                                                    {w.attendance === 'hadir' ? t('invitation.rsvp_hadir') : t('invitation.rsvp_tidak_hadir')}
                                                </span>
                                            )}
                                        </div>
                                        <p className="lx4-wish-bubble__message">{w.message}</p>
                                    </div>
                                ))}
                            </div>
                        </div>
...
function ClosingSection({ invitation, brideGrooms, galleries }) {
    ...
    return (
        <section id="closing" className="lx4-section lx4-closing" style={{ position: 'relative', overflow: 'hidden' }}>
            {globalShowPhotos && bgImg && (
                <>
                    <div className="lx4-section-bg" style={{ backgroundImage: `url(${bgImg})` }} />
                    <div className="lx4-section-overlay" />
            ...
            <div style={{ position: 'relative', zIndex: 2 }}>
                <Reveal>
                    <h2 className="lx4-closing__title">{displayClosingTitle}</h2>
                    <p className="lx4-closing__text">
                        {displayClosingText}
                    </p>
                    <HeartDivider isDark={true} />
                    
                    {/* Formal Tanda Tangan Penutup */}
                    <div className="lx4-closing__signature" style={{ marginTop: '25px', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '8px', marginBottom: '25px' }}>
                        <div style={{ fontWeight: '600', letterSpacing: '1px', textTransform: 'uppercase', fontSize: '0.75rem', color: 'var(--lx4-gold)' }}>
                    ...
                    <h3 className="lx4-closing__couple">{coupleName}</h3>
                    <p style={{ marginTop: 40, fontSize: 10, color: 'var(--lx4-text-muted)', letterSpacing: 1 }}>
...
function Navigation({
    ...
    return (
        <>
            {/* Dock Menu */}
            {showMenu && (
                <div className={`lx4-nav-menu ${isBottomMenu ? 'lx4-nav-menu--bottom' : 'lx4-nav-menu--top'}`}>
                    <div className="lx4-nav-menu__inner--row">
                        {menuItems.map((item) => {
            ...
            <div className={`lx4-floating-btns ${isBottomMenu ? 'lx4-floating-btns--raised' : ''}`}>
                {enableQr && activeGuest && (
                    <button
                        type="button"
                        className="lx4-floating-btn"
                ...
                <button
                    ...
                    style={isFullscreen ? { backgroundColor: 'var(--lx4-gold, #4a0e17)', color: '#fff', boxShadow: '0 0 10px var(--lx4-gold)' } : {}}
                ...
                <button
                    ...
                    style={autoScrollEnabled ? { backgroundColor: 'var(--lx4-gold, #4a0e17)', color: '#fff', boxShadow: '0 0 10px var(--lx4-gold)' } : {}}
            ...
            {enableQr && showQr && activeGuest && (
                <div className="lx4-qr-overlay" onClick={() => setShowQr(false)}>
                    <div className="lx4-qr-modal" onClick={e => e.stopPropagation()}>
                        <h3 className="lx4-qr-title">QR Code Check-in</h3>
                        <p className="lx4-qr-guest">{activeGuest.name}</p>
                        <div className="lx4-qr-img-wrap">
                            <img
                                ...
                                alt="QR Code"
                                className="lx4-qr-img"
                            />
                        </div>
                        <p className="lx4-qr-hint">Tunjukkan QR Code ini kepada penerima tamu di lokasi acara</p>
                        <button
                            type="button"
                            className="lx4-qr-close"
...
export default function DynamicIndex({
    ...
    useEffect(() => {
        ...
        const handleUserInteraction = (e) => {
            if (
                ...
                e.target.closest('.lx4-floating-btn') || 
                e.target.closest('.lx4-nav-menu') || 
    ...
    const handlePointerDown = (clientX, clientY, target) => {
        ...
        if (target && typeof target.closest === 'function') {
            const scrollable = target.closest('.lx4-slide-container');
            if (scrollable) {
    ...
    const handlePointerUp = (clientX, clientY, target) => {
        ...
        if (layoutMode === 'slide-h') {
            ...
            if ((Math.abs(diffY) > Math.abs(diffX)) && (Math.abs(diffY) > threshold || isFastSwipe)) {
                const scrollable = target && typeof target.closest === 'function' ? target.closest('.lx4-slide-container') : null;
                if (scrollable) {
    ...
    const handleWheel = (e) => {
        ...
        const target = e.target.closest('.lx4-slide-container');
    ...
    const renderSectionComponent = (section, idx) => {
        ...
        if (isSlideMode) {
            let slideClass = 'lx4-slide-container';
        ...
        const dividerIsDark = {
            'opening': true,
            'bride_groom': false,
            'event': true, // Event has Countdown at the bottom
            'love_story': false,
            'gallery': false,
            'livestream': true,
            'bank': false,
        ...
        return (
            <React.Fragment key={key}>
                {comp}
                {key !== 'closing' && key !== 'hero' && <HeartDivider isDark={!!dividerIsDark[key]} />}
            </React.Fragment>
        );
    };
    ...
    return (
        <ErrorBoundaryLuxury4>
            <div className={`lx4-page ${!showAnimations ? 'lx4-no-animations' : ''}`}>
            ...
            {/* Main Area */}
            <div className="lx4-container">
                <div
                    className={`lx4-main ${isSlideMode ? 'lx4-main--slide' : ''} ${layoutMode === 'slide-h' ? 'lx4-main--slide-h' : ''} ${layoutMode === 'slide-v' ? 'lx4-main--slide-v' : ''}`}
            ...
            {waNumber && isOpened && (
                <div className={`lx4-whatsapp-float ${isBottomMenu ? 'lx4-whatsapp-float--raised' : ''}`}>
                    <a
                        href={`https://wa.me/${waNumber.replace(/[^0-9]/g, '')}`}
                        target="_blank"
                        rel="noopener noreferrer"
                        className="lx4-whatsapp-btn"
