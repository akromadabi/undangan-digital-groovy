...
export default function Pricing({ plans, currentPlan, features, paymentGatewayType, tripayChannels }) {
    ...
    const handleUpgrade = (planId) => {
        ...
        const greetingCardId = params.get('greeting_card_id');

        const query = { plan_id: planId };
        if (invitationId) query.invitation_id = invitationId;
        if (greetingCardId) query.greeting_card_id = greetingCardId;

        router.get('/checkout', query);
    };
