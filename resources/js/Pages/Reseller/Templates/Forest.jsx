...
const FEATURES = [
    {
        ...
        title: 'Tema Alam & Hijau', desc: 'Pilihan tema dengan perpaduan warna hijau mint segar, dedaunan botanical, dan nuansa tropis yang asri.'
    },
    {
        icon: 'M10.5 1.5H8.25A2.25 2.25 0 006 3.75v16.5a2.25 2.25 0 002.25 2.25h7.5A2.25 2.25 0 0018 20.25V3.75a2.25 2.25 0 00-2.25-2.25H13.5m-3 0V3h3V1.5m-3 0h3m-3 18.75h3',
        title: 'Mobile Friendly', desc: 'Dioptimalkan dengan mulus untuk dibaca di perangkat seluler dengan transisi yang nyaman.'
    },
    {
        ...
        title: 'Konfirmasi RSVP', desc: 'Sistem RSVP cerdas yang mendata kehadiran dengan teratur langsung dari setiap undangan.'
    },
    {
        icon: 'M21.75 6.75v10.5a2.25 2.25 0 01-2.25 2.25h-15a2.25 2.25 0 01-2.25-2.25V6.75m19.5 0A2.25 2.25 0 0019.5 4.5h-15a2.25 2.25 0 00-2.25 2.25m19.5 0v.243a2.25 2.25 0 01-1.07 1.916l-7.5 4.615a2.25 2.25 0 01-2.36 0L3.32 8.91a2.25 2.25 0 01-1.07-1.916V6.75',
        title: 'Sebarkan via WA', desc: 'Mudah membagikan kabar bahagia ke ribuan kontak WhatsApp dengan cepat.'
    },
    {
        ...
        title: 'Amplop Digital', desc: 'Menerima gift dan angpao digital secara praktis dengan transfer instan.'
    },
    {
        icon: 'M9 12.75L11.25 15 15 9.75m-3-7.036A11.959 11.959 0 013.598 6 11.99 11.99 0 003 9.749c0 5.592 3.824 10.29 9 11.623 5.176-1.332 9-6.03 9-11.622 0-1.31-.21-2.571-.598-3.751h-.152c-3.196 0-6.1-1.248-8.25-3.285z',
        title: 'Tamu Unlimited', desc: 'Undang kerabat dan relasi tanpa limitasi jumlah penerima undangan digital.'
    },
];

const STEPS = [
    { num: '01', icon: 'M15.75 6a3.75 3.75 0 11-7.5 0 3.75 3.75 0 017.5 0zM4.501 20.118a7.5 7.5 0 0114.998 0A17.933 17.933 0 0112 21.75c-2.676 0-5.216-.584-7.499-1.632z', title: 'Daftar Gratis', desc: 'Mulai buat akun Anda dengan mudah dan cepat.' },
    ...
    { num: '03', icon: 'M7.217 10.907a2.25 2.25 0 100 2.186m0-2.186c.18.324.283.696.283 1.093s-.103.77-.283 1.093m0-2.186l9.566-5.314m-9.566 7.5l9.566 5.314m0 0a2.25 2.25 0 103.935 2.186 2.25 2.25 0 00-3.935-2.186zm0-12.814a2.25 2.25 0 103.933-2.185 2.25 2.25 0 00-3.933 2.185z', title: 'Sebarkan Undangan', desc: 'Mulai kirimkan link undangan digital estetik Anda kepada kerabat.' },
];

const TESTIMONIALS = [
    { name: 'Nabila & Aris', city: 'Semarang', text: 'Temanya hijau botanical-nya segar sekali di mata! Banyak teman-teman yang memuji keunikannya.', stars: 5 },
    { name: 'Fiona & Zacky', city: 'Malang', text: 'Fitur galeri dan video terintegrasi dengan sangat apik. Sangat puas dengan temanya!', stars: 5 },
];

const FAQS = [
    { q: 'Apakah undangan digital bertema Forest ini bisa ditambahkan peta?', a: 'Tentu saja. Anda bisa langsung mengintegrasikan Google Maps lokasi acara Anda ke dalam undangan.' },
    { q: 'Bagaimana cara mendaftarkan rekening untuk amplop digital?', a: 'Cukup masukkan nomor rekening bank atau e-wallet Anda pada panel dasbor, dan otomatis akan tampil dalam undangan.' }
];

const planMeta = {
    free:     { color: '#10b981', glow: 'rgba(16,185,129,0.1)', label: 'Coba Gratis' },
    silver:   { color: '#34d399', glow: 'rgba(52,211,153,0.15)', label: 'Mulai Sekarang' },
    gold:     { color: '#10b981', glow: 'rgba(16,185,129,0.25)', label: 'Mulai Sekarang', popular: true },
    platinum: { color: '#047857', glow: 'rgba(4,120,87,0.3)', label: 'Mulai Sekarang' },
};

const formatRp = (n) => new Intl.NumberFormat('id-ID', { style: 'currency', currency: 'IDR', minimumFractionDigits: 0 }).format(n);

export default function Forest({ reseller, plans = [], themes = [], greetingCards = [], greetingCardTypeOptions = {}, features = [], sections = [], heroImage = null }) {
    ...
    // Deep Fresh Green Palette
    const T = {
        heroBg: 'linear-gradient(135deg, #051a11 0%, #0c3624 50%, #051a11 100%)',
        accent: '#10b981',
        accentDark: '#047857',
        accentRgb: '16,185,129',
        navBg: 'rgba(5,26,17,0.9)',
        sectionAlt: '#062015',
        sectionBase: '#08291b',
    ...
    const getWhatsappLink = (number, text = 'Halo, saya ingin bertanya tentang undangan digital Forest.') => {
        if (!number) return '#';
    ...
    const renderBanner = (c) => {
        const text = c?.text || '🌿 Nikmati promo paket botanical hemat bulan ini!';
        ...
        return (
            <div className="rl-banner" style={{ background: bgColor, color: textColor }}>
                ...
                />
                {ctaText && (
                    <a href={ctaLink} className="rl-banner__cta" style={{ color: bgColor && bgColor.includes('#') ? bgColor : '#10b981', padding: '0.25rem 0.75rem', background: '#fff', borderRadius: '4px', textDecoration: 'none', fontSize: '0.7rem', fontWeight: 800, marginLeft: '0.5rem', transition: 'all 0.2s' }}>
                        {ctaText}
                    </a>
                )}
    ...
    const renderHero = (c) => {
        const badgeText = c?.badge_text || 'Fresh Botanical & Floral Invitation';
        const title = c?.title || 'Undangan Asri';
        const subtitle = c?.subtitle || 'Forest';
        const description = c?.description || 'Hadirkan tema alami berbalut daun botanical segar untuk menyempurnakan hari bahagia Anda.';
        return (
            <section className="rl-hero" style={activeHeroImage ? { backgroundImage: `linear-gradient(rgba(5,26,17,0.7), rgba(5,26,17,0.85)), url(${activeHeroImage})`, backgroundSize: 'cover', backgroundPosition: 'center' } : {}}>
                ...
                <div className="rl-hero__content">
                    ...
                    <div className="rl-hero__cta-group">
                        <a href={registerUrl} className="rl-cta-btn rl-cta-btn--primary">Mulai Sekarang</a>
    ...
    const renderStats = (c) => {
        const items = c?.items || [
            { val: '20+', label: 'Tema Botanical', icon: '🌿' },
            ...
            <section className="rl-section rl-section--alt">
                <div className="rl-container">
                    <div className="rl-section__header">
                        <span className="rl-section__tag">Proses Pembuatan</span>
                        <h2 className="rl-section__title">Sangat Mudah & Alami</h2>
                    </div>
    ...
    const renderFeatures = (c) => {
        const title = c?.title || 'Fitur Botanical Forest';
            ...
            <section id="preview" className="rl-section rl-section--alt">
                <div className="rl-container">
                    <div className="rl-section__header">
                        ...
                        <h2 className="rl-section__title">Katalog Tema Botanical</h2>
                    </div>
                    {c?.style === 'carousel' ? (
            ...
            <section id="preview-kartu" className="rl-section">
                <div className="rl-container">
                    <div className="rl-section__header">
                        ...
                        <h2 className="rl-section__title">Kartu Ucapan Botanical</h2>
                    </div>
                    {c?.style === 'carousel' ? (
                        <div style={{ display: 'flex', gap: '1.5rem', overflowX: 'auto', padding: '0.5rem 0.5rem 1.5rem', scrollbarWidth: 'none', WebkitOverflowScrolling: 'touch' }}>
    ...
    const renderTestimonials = (c) => {
        return (
            <section className="rl-section rl-section--alt">
                <div className="rl-container">
                    <div className="rl-section__header">
                        <span className="rl-section__tag">Kesan Klien</span>
    ...
    const renderCta = (c) => {
        return (
            <section className="rl-cta-section">
                <div className="rl-container">
                    <h2 className="rl-cta-section__title">Luncurkan Undangan Botanical Eksklusif Anda Sekarang</h2>
    ...
    const siteTitle = reseller.site_title || reseller.brand_name || 'Undangan Forest';
    const siteMotto = reseller.site_motto || 'Platform Undangan Cantik Botanical';
    ...
    return (
        <>
            ...
            <div style={{
                ...
                width: '100%',
                height: '100%',
                background: T.heroBg || T.sectionBase || '#051a11',
                display: 'flex',
                ...
                <div style={{
                    ...
                    <div style={{
                        ...
                        {reseller.brand_name?.charAt(0) || 'U'}
                    </div>
                </div>
                <h3 style={{
                    color: T.textPrimary || '#ecfdf5',
                    fontSize: '18px',
                    fontWeight: '600',
                    letterSpacing: '1px',
                    margin: '0 0 8px 0',
            ...
            <div 
                className="rl-layout-container rl-layout-forest"
                style={{ 
            ...
            <nav className={`rl-nav ${scrolled ? 'rl-nav--scrolled' : ''}`}>
                <div className="rl-nav__inner">
                    ...
                    <div className="rl-nav__actions">
                        ...
                        <a href={registerUrl} className="rl-btn rl-btn--accent">Mulai</a>
                    </div>
                </div>
            </nav>
