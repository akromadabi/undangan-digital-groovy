...
file_path = r'c:\laragon\www\Undangan Digital\resources\js\Pages\GreetingCardPreview.jsx'

with open(file_path, 'r', encoding='utf-8') as f:
    content = f.read()

start_marker = 'function CosmicDriftFull({ card }) {'
end_marker = 'function GreetingCardPreviewContent({ card }) {'
...
replacement_block = """function CosmicDriftFull({ card }) {
    ...
    const floatMessages = useMemo(() => {
        ...
        msgs = list(set(msgs)) if 'list' in globals() else [...new Set(msgs)];
        if (msgs.length === 0) {
            msgs.push(
...
content = content[:start_idx] + replacement_block + content[end_idx:]
